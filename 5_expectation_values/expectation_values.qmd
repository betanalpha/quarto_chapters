...
in Chapter Seven, but for now our main concern will be to avoid
...
colloquially we can say that the two integrals are equal **up to subsets
of measure zero** or equal **up to null subsets**.  I will use this
latter terminology for the remainder of the book.
...
of inputs will yield the same measure-informed intergrals.  In other
words from the perspective of the Lebesgue measure these functions are
equivalent.](
...
injection $\iota : X \rightarrow \mathbb{R}$, in other words a function
that maps each element of $X$ to a distinct output while also preserving
distances,
$$
d_{X}(x_{1}, x_{2})
...
Well-defined moments can be obstructed at three different places.  If
a function $f : X \rightarrow \mathbb{R}$ is not an isometric injective
then it does not define an embedding.  Even if it is, however, we will
not be able to integrate it with respect to a given measure unless it
is both $\mathcal{X}$-measurable and $\mu$-integrable.  To avoid any
...
$\mathbb{R}$ by first cutting the circle into the half-open interval
$(0, 2 \pi]$ and then mapping the half-open interval into a real
line.  The resulting function, however, is not isometric.  Two points
...
real line then we would still run into measurability problems.  It
turns out that any injective function from $\mathbb{S}^{1}$ into
...
considering less conventional $\sigma$-algebras our attempt to construct
moments for any measure on the circle is obstructed once again.
...
[Section 5.2.2](@sec:practical_lebesgue) we'll learn that while the
...
under the curve as a sum of increasingly narrow rectanglular areas, only
