\documentclass{standalone}
\usepackage{graphicx}	
\usepackage{amssymb, amsmath}
\usepackage{color}

\usepackage{tikz}
\usetikzlibrary{intersections, backgrounds, math, arrows.meta}
\usepackage{pgfmath}

\definecolor{light}{RGB}{220, 188, 188}
\definecolor{mid}{RGB}{185, 124, 124}
\definecolor{dark}{RGB}{143, 39, 39}
\definecolor{highlight}{RGB}{180, 31, 180}
\definecolor{lightteal}{RGB}{186, 219, 219}
\definecolor{midteal}{RGB}{124, 186, 186}
\definecolor{darkteal}{RGB}{38, 142, 142}
\definecolor{darkerteal}{RGB}{0, 124, 124}
\definecolor{gray10}{gray}{0.1}
\definecolor{gray20}{gray}{0.2}
\definecolor{gray30}{gray}{0.3}
\definecolor{gray40}{gray}{0.4}
\definecolor{gray60}{gray}{0.6}
\definecolor{gray70}{gray}{0.7}
\definecolor{gray80}{gray}{0.8}
\definecolor{gray90}{gray}{0.9}
\definecolor{gray95}{gray}{0.95}

\tikzmath{
  function vonmises(\x, \m, \k, \logI) {
    return exp( \k * cos(\x - \m) - \logI) / 6.283185307179586;
  };
  function phi(\t, \to, \pa, \pb) {
    if \t >= \to then {
      return \pa + ((\t - \to) / 360) * (\pb - \pa);
    } else {
      return \pa + ((360 + \t - \to) / 360) * (\pb - \pa);
    };
  };
  function logistic(\x) {
    if \x > 0 then {
      return 1 / (1 + exp(-\x));
    } else {
      return 1 * exp(\x) / (1 + exp(\x));
    };
  };
  function unrolledvonmises(\y, \m, \k, \logI) {
    \x = logistic(\y);
    return vonmises(360 * \x, \m, \k, \logI) * \x * (1 - \x);
  };
  function x(\x, \y, \z, \theta, \phi) {
    \xa = +cos(\theta) * \x + sin(\theta) * \z;
    \ya = \y;
    \za = -sin(\theta) * \x + cos(\theta) * \z;
    %
    \xb = \xa;
    \yb = +cos(\phi) * \ya + sin(\phi) * \za;
    %\zb = -sin(\phi) * \ya + cos(\phi) * \za;
    %  
    return \xb;
  };
  function y(\x, \y, \z, \theta, \phi) {
    \xa = +cos(\theta) * \x + sin(\theta) * \z;
    \ya = \y;
    \za = -sin(\theta) * \x + cos(\theta) * \z;
    %
    \xb = \xa;
    \yb = +cos(\phi) * \ya + sin(\phi) * \za;
    %\zb = -sin(\phi) * \ya + cos(\phi) * \za;
    %   
    return \yb;
  };
}

\begin{document}

\begin{tikzpicture}[scale=1.0]

  \pgfmathsetmacro{\r}{1.5}
  \pgfmathsetmacro{\delta}{0.1}
  
  \pgfmathsetmacro{\s}{0.75}
  \pgfmathsetmacro{\mu}{135}
  \pgfmathsetmacro{\kappa}{10}
  \pgfmathsetmacro{\logI}{7.942972}
  
  \pgfmathsetmacro{\theta}{0}
  \pgfmathsetmacro{\phi}{60}

  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-3.5, -2) rectangle (3.5, 1.5);
    
    \pgfmathsetmacro{\R}{1.5}
    \draw[domain={90 * (1 - 2 * \r / \R)}:{90 * (1 + 2 * \r / \R)}, smooth, samples=50, variable=\t, black, line width=1] 
          plot ({x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}, 
                {y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)});
  
    \foreach \n in {1, 2, ..., 12} {
      \pgfmathsetmacro{\t}{ 90 * (1 - 2 * \r / \R) + (\n - 1) * (360 * \r / \R) / 12 }
      \draw[line width=1]    ({x((\R - \delta) * cos(\t), (\r - \R) + (\R - \delta) * sin(\t), 0, \theta, \phi)}, 
                              {y((\R - \delta) * cos(\t), (\r - \R) + (\R - \delta) * sin(\t), 0, \theta, \phi)})
                          -- ({x((\R + \delta) * cos(\t), (\r - \R) + (\R + \delta) * sin(\t), 0, \theta, \phi)}, 
                              {y((\R + \delta) * cos(\t), (\r - \R) + (\R + \delta) * sin(\t), 0, \theta, \phi)});
    }

    \pgfmathsetmacro{\t}{90 * (1 - 2 * \r / \R)}     
    \pgfmathsetmacro{\x}{x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}
    \pgfmathsetmacro{\y}{y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)})
    
    \draw[gray80, line width=1, dashed] (\x, \y) -- +(0, 2);
    
    \pgfmathsetmacro{\t}{90 * (1 + 2 * \r / \R)}     
    \pgfmathsetmacro{\x}{x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}
    \pgfmathsetmacro{\y}{y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)})
    
    \draw[gray80, line width=1, dashed] (\x, \y) -- +(0, 2); 

    \pgfmathsetmacro{\t}{90 + (180 - 22) * \r / \R)}
    \fill[white] ({x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}, 
                  {y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}) circle (0.05);    
    \fill[darkteal] ({x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}, 
                 {y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}) circle (0.04)
    node[below] { $x_{1}$ }; 

    \pgfmathsetmacro{\t}{90 - (180 - 22) * \r / \R)}
    \fill[white] ({x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}, 
                  {y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}) circle (0.05);
    \fill[darkteal] ({x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}, 
                 {y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}) circle (0.04)
    node[below] { $x_{2}$ }; 
    
    \node at (0, -1.5) { $\mathbb{S}^{1}$ };        
  \end{scope}
  
  \begin{scope}[shift={(0, -3.5)}]
    \draw[white] (-3.5, -2) rectangle (3.5, 1.5);
    
    \pgfmathsetmacro{\R}{2}
    \draw[arrows={Bracket[sharp]-Parenthesis[sharp]}, 
          domain={90 * (1 - 2 * \r / \R)}:{90 * (1 + 2 * \r / \R)}, 
          smooth, samples=50, variable=\t, black, line width=1] 
          plot ({x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}, 
                {y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)});
  
    \foreach \n in {2, 3, ..., 12} {
      \pgfmathsetmacro{\t}{ 90 * (1 - 2 * \r / \R) + (\n - 1) * (360 * \r / \R) / 12 }
      \draw[line width=1]    ({x((\R - \delta) * cos(\t), (\r - \R) + (\R - \delta) * sin(\t), 0, \theta, \phi)}, 
                              {y((\R - \delta) * cos(\t), (\r - \R) + (\R - \delta) * sin(\t), 0, \theta, \phi)})
                          -- ({x((\R + \delta) * cos(\t), (\r - \R) + (\R + \delta) * sin(\t), 0, \theta, \phi)}, 
                              {y((\R + \delta) * cos(\t), (\r - \R) + (\R + \delta) * sin(\t), 0, \theta, \phi)});
    }

    \pgfmathsetmacro{\t}{90 * (1 - 2 * \r / \R)}     
    \pgfmathsetmacro{\x}{x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}
    \pgfmathsetmacro{\y}{y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)})
    
    \draw[gray80, line width=1, dashed] (\x, \y) -- +(0, 2);
    
    \pgfmathsetmacro{\t}{90 * (1 + 2 * \r / \R)}     
    \pgfmathsetmacro{\x}{x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}
    \pgfmathsetmacro{\y}{y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)})
    
    \draw[gray80, line width=1, dashed] (\x, \y) -- +(0, 2); 
    
    \pgfmathsetmacro{\t}{90 + (180 - 22) * \r / \R)}
    \fill[white] ({x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}, 
                  {y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}) circle (0.05);    
    \fill[darkteal] ({x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}, 
                 {y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}) circle (0.04)
    node[below left] { $y_{1}$ }; 

    \pgfmathsetmacro{\t}{90 - (180 - 22) * \r / \R)}
    \fill[white] ({x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}, 
                  {y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}) circle (0.05);
    \fill[darkteal] ({x(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}, 
                 {y(\R * cos(\t), (\r - \R) + \R * sin(\t), 0, \theta, \phi)}) circle (0.04)
    node[below right] { $y_{2}$ }; 
    
    \node at (0, -1.5) { $(0, 2 \pi]$ };  
  \end{scope}
  
  \begin{scope}[shift={(0, -7)}]
    \draw[white] (-3.5, -2) rectangle (3.5, 1.5);
    
    \begin{scope}[shift={(0, -0.5)}]
      \draw [<->, >=stealth, line width=1] (-3.00, 0) -- +(6, 0);
      
      \foreach \t in {30, 60, ..., 330} {
        \draw[line width=1]    ( { ln(\t / (360 - \t) }, { -\delta } )
                            -- ( { ln(\t / (360 - \t) }, { +\delta } );
      }
      
      \pgfmathsetmacro{\t}{22}
      \fill[white] ( { ln(\t / (360 - \t) }, 0 ) circle (0.05);
      \fill[darkteal] ( { ln(\t / (360 - \t) }, 0 ) circle (0.05) circle (0.04)
      node[below] { $z_{1}$ }; 
      
      \pgfmathsetmacro{\t}{338}
      \fill[white] ( { ln(\t / (360 - \t) }, 0 ) circle (0.05);
      \fill[darkteal] ( { ln(\t / (360 - \t) }, 0 ) circle (0.05) circle (0.04)
      node[below] { $z_{2}$ }; 
        
    \end{scope}
    
    \node at (0, -1.5) { $\mathbb{R}$ }; 
  \end{scope}
  
\end{tikzpicture}

\end{document}  