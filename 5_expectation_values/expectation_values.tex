...
\hypersetup{
  pdftitle={Measured-Informed Integration and Expectation},
...
\title{Measured-Informed Integration and Expectation}
...
is available on
\href{https://github.com/betanalpha/quarto_chapters/tree/main/5_expectation_values}{GitHub}.
\end{abstract}
...
Four}, we defined measures and probability distributions, as mappings
from measurable subsets to allocated measures. These subset allocations
also induce a somewhat surprising, but incredibly powerful, mapping from
real-valued functions to single real numbers that generalizes Riemann
integration in calculus. This measure-informed integration operation
...
without having to explicitly define subset allocations, as well as some
useful applications of these implicit specifications. Finally, we'll
consider particular measure-informed integrals that are distinguished by
common ambient space structures before concluding with a discussion of a
few exceptional measures whose integrals can be computed
algorithmically.

\section{Integration on Finite Measure
Spaces}\label{sec:finite_integration}
...
numerical output. Because the output of both \(\mu\) and \(f\) are
real-valued, we can also multiply them together into weighted outputs,
\[
\mu(x) \cdot f(x).
\]
...
\end{align*} This scalar summary emphasizes not only large output values
but also outputs from high-measure inputs. For example, even if the
output \(f(\Box)\) is small, the contribution from \(\Box\) can still be
important to the weighted sum if the atomic allocation \(\mu(\Box)\) is
large.
...
\end{align*} We use square brackets instead of round brackets here to
visually indicate that the mapping doesn't take points as input but
rather entire functions.

An interesting side effect of this construction is that measure-informed
integrals are always linear. Given two real-valued functions
...
considering multiple \emph{test} measures, we can use this operation to
more fully probe the behavior of a fixed function \(f\).
...
intuitively, we can interpret each test measure \(\mu_{j}\) as encoding
a question about \(f\), with the corresponding measure-informed integral
\(\mathbb{I}_{\mu_{j}}[f]\) encoding the answer
...
\captionsetup{labelsep=none}\includegraphics{figures/probing_functions/probing_functions.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/probing_measures/probing_measures.pdf}
...
Consider, for instance, a \emph{singular} probability mass function that
...
with respect to \(\delta_{x'}\) filters out individual output values
\(f(x')\).
...
the measures \(\mu\) allocates to those inputs. Again, we can interpret
...
\end{array}
\right. .
\] Unsurprisingly, an indicator function \emph{indicates} whether or not
...
\\
&= \mu( \mathsf{x} ).
\end{align*} Measure-informed integrals of indicator functions allow us
to directly probe measurable subset allocations.

\section{Integration on General Measure Spaces}\label{sec:gen_int}
...
On uncountable spaces, for example, element-wise allocations
\(\mu( \{x\} )\) do not completely characterize a measure. Consequently,
the weighted output values \(\mu( \{ x \} ) \cdot f(x)\) cannot
completely characterize the interaction between a measure and a
...
To start, let's consider indicator functions which vanish outside of a
given measurable subset (Figure~\ref{fig-indicator}) \[
I_{\mathsf{x}} (x) =
\left\{
\begin{array}{rr}
1, & x \in \mathsf{x} \\
...
\includegraphics[width=0.6\textwidth,height=\textheight]{figures/indicator_function/indicator_function.pdf}
...
encountered in \hyperref[sec:finite_integration]{Section One}, the
...
\] is any collection of real numbers. By incorporating countably many
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/simple_function/simple_function.pdf}
...
however, can often be \emph{well-approximated} by sufficiently many
simple functions. As we incorporate more and more indicator functions,
we can construct simple functions that approximate non-simple functions
better and better (Figure~\ref{fig-simple-approx}).

\begin{figure}

\centering{

\includegraphics[width=0.9\textwidth,height=\textheight]{figures/simple_function_approx/simple_function_approx.pdf}
...
non-measurable subsets; consequently measurability can largely be taken
for granted.
...
Seven}. For now, our main concern will be to avoid confusing measurable
\emph{subsets} on \((X, \mathcal{X})\) and measurable \emph{functions}
...
exceptional among all of the functions that might come up in a given
analysis. To define measure-informed integrals for measurable functions
that might be negative, we just have to decompose the functions by the
sign of their outputs (Figure~\ref{fig-sign-decomposition}), \[
f(x) = f^{+}(x) - f^{-}(x),
\] where \[
f^{+} (x) =
\left\{
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/sign_decomposition/sign_decomposition.pdf}
...
of its output values, resulting in the two non-negative functions
...
To avoid excessive verbiage, I will refer to any real-valued function
\(f: X \rightarrow \mathbb{R}\) that is measurable with respect to the
ambient \(\sigma\)-algebra and integrable with respect to any relevant
measures simply as an \textbf{integrand}.
...
enough exceptions that we do have to be a bit careful to explicitly
...
other hand, the corresponding measure-informed integrals will differ
only if \[
\mu(\mathsf{x}') > 0!
\] In other words, if \(\mathsf{x}'\) is a \(\mu\)-null subset then
\(s\) and \(s'\) will share the exact same \(\mu\)-integrals.
...
\] with \[
\mu(\mathsf{x}) = 0.
\] Intuitively, modifying integrands on subsets of measure zero does not
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/equivalent_integrands/equivalent_integrands.pdf}
...
equivalent integrands, only one will be continuous. In other words, even
...
When working on a single space, this isn't too much of an issue. That
said, it can cause confusion when we start working with multiple spaces
at the same time.
...
integration in detail in \hyperref[sec:integration_on_R]{Section 5.2}.
...
In probability theory we are interested in not general measures, but
rather probability distributions. General measures are used only as
tools to help implement probability distributions in practice.
...
integrals, however, can also be used to directly define measures, with
...
\end{alignat*} Because not every real-valued function has a well-defined
measure-informed integral, however, this isn't technically correct. In
other words, \(C(X)\) is too large of a space of functions.
...
\emph{linear} mapping between these spaces.
...
to every measurable subset, and hence fully determines the measure
...
integrals of other integrands, including indicator functions from which
we can recover subset allocations, can then be derived from these
initial integrals. These sparse characterizations are particularly
useful for analyzing certain theoretical properties of measures with the
tools of \textbf{functional analysis}.
...
only what operations we need to carry out in an applied analysis, but
also how well our algorithmic tools actually implement those operations.
We will spend a good bit of time discussing these issues in later
chapters.
...
\end{align*} In particular, the modified subset allocations are no
longer given by simple scalings of the initial subset allocations!
...
&=
\mathbb{E}_{\pi} [ g ]
\end{align*} which is not, in general, equal to \(1\). Scaling a
probability distribution usually results not in another probability
distribution, but rather a generic measure!

If we want to transform one probability distribution into another, then
we need to correct for the modified normalization. To that end, we can
define the renormalized scaling \(g \ast \pi\) as the probability
distribution with the expectation values \begin{align*}
...
For instance, scaling with an indicator function restricts a probability
distribution to the corresponding subset while also reducing the total
...
allocations to account for this restriction, \begin{align*}
...
compatible with the structure of an ambient space; their integrals
...
distinguished integrand. We can build off of this initial integrand to
construct an entire family of useful integrands.

\subsubsection{Embeddings}\label{embeddings}

In order for an ambient space \(X\) to be compatible with a real line,
it needs to share the metric structure of the real line. We say that we
can \textbf{embed} a metric space \(X\) into a real line if we can
construct an isometric injection \(\iota : X \rightarrow \mathbb{R}\),
in other words a function that maps each element of \(X\) to a distinct
output while also preserving distances, \[
d_{X}(x_{1}, x_{2})
...
Note that the construction of an embedding requires fixing the output
real line to be a particular rigid real line or, equivalently, a
particular parameterization of a flexible real line. An ambient space
might embed into \emph{a} real line, but it cannot embed into \emph{all}
real lines at the same time.

If \(X\) is itself a real line, then the identify map defines a natural
embedding \(\iota : \mathbb{R} \hookrightarrow \mathbb{R}\)
...
\captionsetup{labelsep=none}\includegraphics{figures/embeddings/identity/identity.pdf}

}

\subcaption{\label{fig-identity}}
...
\captionsetup{labelsep=none}\includegraphics{figures/embeddings/interval/interval.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/embeddings/integer/integer.pdf}
...
line, including (a) that real line itself, (b) intervals of that real
line, and (c) integers.}
...
a real line, if not an entire real line. This allows us to interpret
\(X\) as being that subset. Alternatively, we can think of an embedding
map as assigning to each element \(x \in X\) a numerical \emph{position}
that we can use to characterize geometric behavior. Both interpretations
are useful, but in this section we will lean heavily on this latter
perspective.

When an embedding map is measurable with respect to the ambient
\(\sigma\)-algebra and integrable with respect to the ambient measure,
it defines an integrand. Most embedding maps are measurable.
Integrability, however, is less dependable, and failures of
integrability are important in practice.
...
any measure with a mass function. In this case, we can explicitly
compute \(\mathbb{I}_{\mu}[\iota]\) as a weighted sum of positions, \[
...
So long as \(\iota : X \hookrightarrow \mathbb{R}\) is an embedding,
this squared distance function will be measurable and, consequently,
will define a valid integrand. The measure-informed integral
\begin{align*}
...
that minimizes the integrated squared distance using calculus tools even
if \(X\) itself is not continuous.
...
Specifically, the measure-informed integral of the embedding function
\(\mathbb{I}_{\mu} [ \iota ]\) is exactly the position that minimizes
the expected squared distance. In that sense, it is the position closest
to the concentration of \(\mu\). Note, however, that if \(X\) is not
continuous, for example if \(X = \mathbb{Z}\), then this central
position might fall between the positions of the individual elements
(Figure~\ref{fig-mean}).
...
\includegraphics[width=0.7\textwidth,height=\textheight]{figures/mean/mean.pdf}

}

\caption{\label{fig-mean}The measure-informed integral of an embedding
function, \(\mathbb{I}_{\mu} [ \iota ]\), can take arbitrary real values
even when the ambient space cannot. In these cases, the centrality of a
measure can fall ``between'' the individual elements.}

\end{figure}%

Because \(\mathbb{I}_{\mu} \left[ \iota \right]\) quantifies a sense of
the centrality of the measure \(\mu\), it is referred to as the
\textbf{mean} of \(\mu\), in reference to the ``middle'' of the measure.
When space is at a premium, I will use \(\mathbb{M}_{\mu}\) to denote
the mean, with the ambient space and embedding map all implicit.
...
diffuse the concentration is. This is known as the \textbf{variance} of
\(\mu\).
...
embedding function can be systematized in various ways. The direct
powers, \[
...
cumulants is complicated, with some very interesting connections to
combinatorics, but in this book we'll focus on only the first few
cumulants. Conveniently the first-order cumulant is just the mean, \[
...
Well-defined moments can be obstructed in three different ways. First,
an isometric injection from the ambient space into a real line might not
exist. Second, if an isometric injection does exist then it needs to be
...
\includegraphics[width=0.4\textwidth,height=\textheight]{figures/embeddings/circle/circle.pdf}
...
an isometric injection, we cannot define the standard moments for any
measure over a circle.}

\end{figure}%

Formally, one can use the topological incompatibility between the circle
and the real line that we first encountered in
\href{http://www.symplectomorphic.com/assets/chapters_html/spaces.html}{Chapter
...
conditions when working with more familiar spaces as well. For example,
in \hyperref[sec:practical_lebesgue]{Section 5.2.2} we'll learn that the
...
Lebesgue measure does not have a well-defined mean, let alone a variance
...
For instance, an ordering over an ambient space motivates interval
subsets, such as the half-open interval subsets \[
( x_{1}, x_{2} ] = \{ x \in X \mid x_{1} < x \le x_{2} \}.
\] We can use \emph{disjoint} intervals to study the behavior of a
measure by investigating how the measure allocations, or equivalently
the measure-informed integrals of the corresponding indicator functions,
vary across the ambient space.

More formally, given the sequence of points \[
\{ x_{1}, \ldots, x_{b}, \ldots, x_{B + 1} \} \in X
\] we can partition the half-open interval \((x_{1}, x_{B + 1}]\) into a
sequence of \(B\) disjoint half-open intervals, \begin{align*}
...
\includegraphics[width=0.6\textwidth,height=\textheight]{figures/histograms/histogram/histogram.pdf}
...
key features of a measure (Figure~\ref{fig-hist-examples}). For
instance, histograms allow us to differentiate between allocations that
concentrate around a point, referred to as \textbf{unimodal} measures,
and allocations that concentrate around multiple points, referred to as
\textbf{multimodal} measures. At the same time, we can see \emph{how} a
measure concentrates around a point, for example whether the
...
\captionsetup{labelsep=none}\includegraphics{figures/histograms/varying_behaviors/diffuse/diffuse.pdf}

}

\subcaption{\label{fig-hist-diffuse}}
...
\captionsetup{labelsep=none}\includegraphics{figures/histograms/varying_behaviors/unimodal/unimodal.pdf}

}

\subcaption{\label{fig-hist-unimodal}}
...
\captionsetup{labelsep=none}\includegraphics{figures/histograms/varying_behaviors/unimodal_skewed/unimodal_skewed.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/histograms/varying_behaviors/multimodal/multimodal.pdf}
...
\caption{\label{fig-hist-examples}Histograms are extremely effective at
...
distinct points. Finally, the measure in (d) concentrates around a
single point, but that concentration is strongly asymmetric, unlike the
concentration in (b).}

\end{figure}%

The smaller the bins, the finer the features we can resolve
(Figure~\ref{fig-hist-binning}). At the same time, the smaller the bins,
the more measure-informed integrals we have to compute in order to
construct the full histogram. In practice, we have to choose a binning
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/histograms/varying_binning/varying_binning.pdf}
...
expensive to construct. As we use smaller bins here, we start to resolve
...
heights are scaled to accommodate the smaller measures and facilitate
the comparison between the three histograms.}

\end{figure}%

The practical limitation to a finite number of bins also the choice of
histogram boundaries important. Because a histogram censors any behavior
below \(x_{1}\) and above \(x_{B + 1}\), we need to choose the binning
to span all of the behaviors of interest. If, for example, the ambient
measure allocations decay towards smaller and larger values, then we can
set the bin boundaries to where the allocations start to become
negligible.

On discrete measure spaces, we can always tune the bins in a histogram
to span only a single element. In this case, the height of each bin
reduces to \(\mu( \{ x \} )\), and the resulting histogram reduces to a
...
quantifies how the measure \emph{accumulates} as we scan across the
space, \begin{alignat*}{6}
M :\; & X & &\rightarrow& \; &[0, \mu(X)]&
\\
& x & &\mapsto& & M_{\mu}(x) = \mu(\mathsf{I}_{x}) = \mathbb{I}_{\mu}[I_{\mathsf{I}_{x}}] &.
\end{alignat*} Accordingly, this mapping is known as a
\textbf{cumulative distribution function} (Figure~\ref{fig-cdf-basics}).

\begin{figure}

\centering{

\includegraphics[width=0.5\textwidth,height=\textheight]{figures/cdfs/cdf/cdf.pdf}
...
points and the cumulative distribution function always returns zero. As
we move towards larger values, the interval expands and accumulates more
and more measure. Finally, at the upper boundary of the space the
interval asymptotes to the total measure.}

\end{figure}%

Using the shorthand notation for implicit subset allocations that we
discussed in
\href{http://www.symplectomorphic.com/assets/chapters_html/probability_on_general_spaces.html}{Chapter
Four}, we can also write a cumulative distribution functions as
\(\mu([x' < x])\) or even \(\mu(x' < x)\). That said, the shorthand in
this case can be confusing because it's easy to mistake which variable
denotes points in the interval and which variable defines the upper
...
features of the ambient measure. For instance, if the measure
...
before and after (Figure~\ref{fig-cdf-unimodal}). In general, the faster
the cumulative distribution function increases in a neighborhood the
stronger the concentration will be there
(Figure~\ref{fig-cdf-narrow-unimodal}). Similarly, if there are any gaps
in the allocation, where intermediate intervals are allocated zero
measure, then the cumulative distribution function will flatten out
completely (Figure~\ref{fig-cdf-gap}).
...
\captionsetup{labelsep=none}\includegraphics{figures/cdfs/cdf_behaviors/unimodal/unimodal.pdf}

}

\subcaption{\label{fig-cdf-unimodal}}
...
\captionsetup{labelsep=none}\includegraphics{figures/cdfs/cdf_behaviors/narrow_unimodal/narrow_unimodal.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/cdfs/cdf_behaviors/gap/gap.pdf}
...
\caption{\label{fig-cdf-examples}A careful survey of the shape of a
cumulative distribution function can communicate a wealth of information
about the ambient measure. (a) Here the ambient measure is unimodal with
the cumulative distribution function appreciably increasing only once we
...
flattens if there are any gaps in the measure allocation. In this case,
the measure concentrates around two points separated by null interval
\(\mathsf{n}\) in between.}

\end{figure}%

One really nice feature of cumulative distribution functions is that
they can be used to directly calculate interval probabilities. The union
of any two-sided, half-open interval \[
( x_{1}, x_{2} ] = \{ x \in X \mid x_{1} < x \le x_{2} \}
...
\] Because measure allocations to disjoint subsets are additive, this
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/cdfs/cdf_probs/cdf_probs.pdf}
...
half-open interval spanning those two points. This allows us to readily
calculate interval measure allocations.}
...
words, the cumulative distribution function will in this case
\emph{completely} characterizes the measure. Consequently, it can be
considered an alternative way to define measures entirely. On every
ordered measurable space that we will encounter, such spaces of integers
and real numbers equipped with Borel \(\sigma\)-algebras, the
equivalence of measures and cumulative distribution functions will be
true.
...
\includegraphics[width=0.8\textwidth,height=\textheight]{figures/cdfs/discrete_cdf/discrete_cdf.pdf}
...
Mass functions, however, do not completely define a measure on ordered
...
jumps in a cumulative distribution function corresponds to individual
...
\includegraphics[width=0.6\textwidth,height=\textheight]{figures/cdfs/jump/jump.pdf}

}

\caption{\label{fig-cdf-jump}When the ambient space is ordered but
uncountable and every atomic subset is a null subset, the cumulative
...
\] we can invert it. The inverse function maps any accumulated measure
...
\includegraphics[width=0.8\textwidth,height=\textheight]{figures/quantiles/quantile_function/quantile_function.pdf}

}

\caption{\label{fig-quantile-function}If a cumulative distribution
function is invertible, then its inverse defines a quantile function
that maps accumulated measures to the points in the ambient space where
the accumulation is reached.}

\end{figure}%

Because quantiles of probability distribution functions are particularly
useful in many applications, they are often given explicit names. For
...
other by quantifying slightly different notions of centrality.
Similarly, the points where a quarter of the probability has been
accumulated and a quarter of the probability remains, \begin{align*}
\Pi(x_{0.25}) &= 0.25
\\
\Pi(x_{0.75}) &= 0.75,
...
quantile function will not be well-defined. For instance, on a countable
...
\includegraphics[width=0.6\textwidth,height=\textheight]{figures/quantiles/inverse_problems/inverse_problems.pdf}
...
\(x_{m-}\) and \(x_{m+}\) when the cumulative distribution function is
not invertible. In this case, different interpolation strategies define
different quantile functions.

\section{Explicit Measure-Informed Integrals}\label{sec:exp_int}

Up to this point, our discussion of measure-informed integration has
been theoretical. Given a measure \(\mu\), we have shown that a linear
map \(\mathbb{I}_{\mu}[f] \in \mathbb{R}\) from sufficiently nice,
real-valued functions \(f\) to real numbers is well-defined. We do not
yet know, however, how to \emph{evaluate} that map to give explicit
measure-informed integrals in practice.

Fortunately, a few exceptional measures produce integrals that can be
computed from certain explicit mathematical operations. This allow us to
...
to implement in practice. Measure-informed integrals on these spaces can
also be computed from the outputs of the mass function.

\subsubsection{Integration As Summation}\label{integration-as-summation}
...
by applying first linearity, \begin{align*}
\mathbb{I}_{\mu}[f]
&\equiv
...
In other words, measure-informed integrals with respect to \emph{any}
...
completely determined by atomic allocations, they can be computed using
only the mass function and not the entire measure.
...
construction that we considered in \hyperref[finite_expectation]{Section
1}.
...
measure-informed integral by directly looping over each point in the
space. Unfortunately, direct summations becomes unfeasible if \(X\)
contains a countably infinite number of elements.

Some infinite sums do enjoy closed-form solutions. For most infinite
sums, however, we cannot evaluate the corresponding measure-informed
integral exactly. That said, we may be able to \emph{approximate} them
by summing over only a finite number of elements where \(\mu(x)\) is, or
perhaps both \(\mu(x)\) and \(f(x)\) are, large. The more terms we
include, the better these finite sums will approximate the exact
measure-informed integrals.

Consider, for example, the counting measure that allocates unit measure
to each atomic subset, \[
\chi( \{x \}) = 1.
\] More generally, the counting measure allocates measure by counting
the number of elements contained in a given subset, \[
...
in \hyperref[sec:scaling_measures]{Section 3.2}? The scaled measure
...
\end{align*} This, however, is also the measure-informed integral of the
simple function itself (Figure~\ref{fig-simple-area}) \begin{align*}
\text{area}
&=
\sum_{j}
...
\captionsetup{labelsep=none}\includegraphics{figures/integration/lebesgue_indicator/lebesgue_indicator.pdf}

}

\subcaption{\label{fig-indicator-area}}
...
\captionsetup{labelsep=none}\includegraphics{figures/integration/lebesgue_simple/lebesgue_simple.pdf}
...
to the Lebesgue measure are equal to the area under the curves defined
by those simple functions. (a) The area under the curve defined by an
interval indicator function is equal to the height, \(1\), times the
length of the interval. That, however, is just equal to the Lebesgue
integral of the indicator function itself. (b) The area under the curve
defined by a more general simple functions is built up from the area of
...
hand, we can use the decomposition to define general Lebesgue integrals
...
At the same time, we can use calculus to compute the area under the
curves defined by a sufficiently nice function
\(f: \mathbb{R} \rightarrow \mathbb{R}\). A Riemann integral is defined
by partitioning the real line into equally-sized intervals and then
constructing rectangles from the height of the integrand at the end of
each interval. As the interval length \(\delta\) becomes smaller and
smaller, the sum of the rectangle areas converges to the area under the
curve (Figure~\ref{fig-riemann}), \[
\int \mathrm{d} x \, f(x)
=
...
\captionsetup{labelsep=none}\includegraphics{figures/integration/lebesgue/lebesgue.pdf}

}

\subcaption{\label{fig-lebesgue}}
...
\captionsetup{labelsep=none}\includegraphics{figures/integration/riemann/riemann.pdf}
...
converges to the Lebesgue integral of \(f\). At the same time, the sum
of the rectangular areas defined by each component indicator function
...
Geometrically, we can think of Lebesgue integration as computing the
area under a curve by summing over \emph{vertically} stacked rectangles,
while Riemann integration computes the area by summing over
\emph{horizontally} stacked rectangles. Riemann integration doesn't
always result in a well-defined answer, but when it does it will be
consistent with the corresponding Lebesgue integral. Consequently, class
integration to compute Lebesgue integrals.
...
\(\int \mathrm{d} x \, f(x)\) is well-defined. Lebesgue integration on
the real line completely generalizes Riemann integration;
measure-informed integration then generalizes Lebesgue integration on
the real line to arbitrary measures and spaces.

This equivalence is the motivation for the many alternative notations
that we discussed in \hyperref[sec:alt_notations]{Section 2.4}. In
general, the integral signs in those notations do not correspond to the
Riemann integral of calculus, but in the special case of the Lebesgue
measure over a real line with sufficiently nice integrands they do!

\subsubsection{Practical Consequences}\label{sec:practical_lebesgue}

When a real-valued function has a well-defined Riemann integral, we can
apply the tools of calculus to evaluate Lebesgue integrals. The
...
Note that the correct, positive answer requires integrating from the
lower end of the interval to the upper end. Changing the order defines
the same interval, and hence the same Lebesgue measure allocation, but
it flips the sign of the Riemann integral. In order to properly relate
Lebesgue integrals to Riemann integrals, we have to fix the
\emph{orientation} of the intervals. To avoid any sign errors, it never
hurts to add absolute values for safety, \[
\mathbb{I}_{\lambda}[I_{[x_{1}, x_{2}]}]
=
...
The problem here is not that the Riemann integral is ill-defined, but
rather that the Lebesgue integral itself is ill-defined. In particular,
the identify function is not Lebesgue-integrable, \begin{align*}
\mathbb{I}_{\lambda}[| \iota |]
&=
\int_{-\infty}^{\infty} \mathrm{d} x \, | \iota(x) |
\\
...
\(g : X \rightarrow \mathbb{R}^{+}\) can be implemented with the Riemann
...
\end{align*} In this case, the measure allocated to any interval becomes
\begin{align*}
...
procedure in the
\href{http://www.symplectomorphic.com/assets/chapters_html/density_functions.html}{next
chapter}.
...
values are the main way that we probe the behavior of probability
distributions. Indeed, a recurring theme in applied probability theory
will be the principled estimation of expectation values for relevant
expectands.
...
measures, including many probability distributions. Later on, we'll
learn some powerful \emph{sampling} techniques for directly estimating
expectation values for general probability distributions.

\section*{Appendix: Indicator Function Manipulations}\label{sec:app}
\addcontentsline{toc}{section}{Appendix: Indicator Function
Manipulations}

In \hyperref[sec:gen_int]{Section 2}, we used indicator functions in the
theoretical construction of measure-informed integration. As we saw in
\hyperref[sec:exp_int]{Section 5}, however, we don't actually use
indicator functions when actually calculating most measure-informed
integrals in practice. Consequently, indicators don't play a direct role
in most practical integration.

Every now and then, however, indicator functions are useful beyond just
integration. In those exceptional circumstances, familiarity with the
properties of indicator functions can be pretty useful. The most useful
of these properties are really just manifestations of the interaction
between probability allocations and the basic set operations that we
discussed in
\href{http://www.symplectomorphic.com/assets/chapters_html/probability_on_general_spaces.html\#derived-properties}{Chapter
4}.

Consider, for example, the indicator function of the union of two
\emph{disjoint} subsets, \[
...
input \(x\) can only be in one subset at a time. This allows us to
decompose the indicator function into two additive pieces,
\begin{align*}
I_{ \mathsf{x}_{1} \cup \mathsf{x}_{2} } (x)
&=
\left\{
...
One immediate consequence of this is that the indicator function of a
set complement is equal to the reflection of the indicator function of
the corresponding set, \begin{align*}
I_{\mathsf{x}} (x)
+
I_{\mathsf{x}^{c}} (x)
&=
...
To generalize this result to non-disjoint subsets, we have to first
decompose each subset and their intersection into disjoint pieces. Let's
use \(\mathsf{x}_{1 \setminus 2}\) to denote the elements in
\(\mathsf{x}_{1}\) that are not also in \(\mathsf{x}_{2}\), with
\(\mathsf{x}_{2 \setminus 1}\) denoting the elements in
\(\mathsf{x}_{2}\) that are not also in \(\mathsf{x}_{1}\). This allows
us to write \begin{align*}
\mathsf{x}_{1}
&=
...
Substitution this result into the third equation gives \begin{align*}
I_{ \mathsf{x}_{1 \setminus 2} } (x)
+
I_{ \mathsf{x}_{1} \cap \mathsf{x}_{2} } (x)
+
...
+
I_{ \mathsf{x}_{1} \cup \mathsf{x}_{2} } (x).
\end{align*} Intuitively, the sum of indicator functions of overlapping
subsets double counts the inputs that fall into both subsets.

In addition to these additive relationships, indicator functions also
satisfy a multiplicative relationship. For any input point \(x\), the
product of the outputs of any two indicator functions, indicator
functions \[
I_{\mathsf{x}_{1}} (x)
\cdot
I_{\mathsf{x}_{2}} (x),
\] will be non-zero if, and only if, both of the outputs are non-zero.
This requires that the \(x\) falls into both \(\mathsf{x}_{1}\) and
\(\mathsf{x}_{2}\); in other words \(x\) must be in the
\emph{intersection} of the two subsets. Consequently, \[
I_{\mathsf{x}_{1}} (x)
\cdot
I_{\mathsf{x}_{2}} (x)
=
I_{\mathsf{x}_{1} \cap \mathsf{x}_{2}} (x).
\] This allows us to convert from point-wise multiplication to set
intersections and back whenever it might be convenient.

As a special case, the point-wise product of an indicator function with
itself is given by \begin{align*}
I^{2}_{\mathsf{x}} (x)
&=
I_{\mathsf{x}} (x)
\cdot
...
&=
I_{\mathsf{x}} (x).
\end{align*} Formally, this property is known as idempotence under
point-wise multiplication.

\section*{Acknowledgements}\label{acknowledgements}
\addcontentsline{toc}{section}{Acknowledgements}

I thank Simon Duane, Pietro Monticone, and Léo Burgund for helpful
comments.
