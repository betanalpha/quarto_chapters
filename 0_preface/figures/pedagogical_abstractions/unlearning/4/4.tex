...
\begin{tikzpicture}[scale=1, thick]
  ...
  \fill[dark] ({- 2 * cos(-\phi) + 1.25}, {-2 * sin(\phi)}) arc ({180 + \phi}:{180 - \phi}:2)
           -- ({1.75 * cos(\theta) - 1.25}, {1.75 * sin(\theta)}) arc (\theta:{360-\theta}:1.75);
  
  \node[dark, align=center] at (0.7, 0) { Deprecated\\Understanding };
