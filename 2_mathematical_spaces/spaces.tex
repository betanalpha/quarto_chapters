...
is available on
\href{https://github.com/betanalpha/quarto_chapters/tree/main/2_mathematical_spaces}{GitHub}.
\end{abstract}
...
\href{http://www.symplectomorphic.com/assets/chapters_html/probability_on_finite_sets.html}{Chapter
One}; this simple setting allowed us to focus on the basic concepts
without too much mathematical distraction. Unfortunately, finite sets
have only limited use in practical applications. Most applications
typically require probability theory on not only more sophisticated sets
but also sets that are equipped with additional structure. These
combinations of sets and structure are also known as \emph{spaces}.

In this chapter we will discuss the basic conceptual features of
general, mathematical spaces before reviewing some of the prototypical
spaces that are particularly common in practical applications. This
presentation will include not only the properties of sets with an
arbitrary number of elements but also a survey of some of the most
fundamental structures that we can endow onto those sets.

\section{Spaces}\label{spaces}

In both more applied and more pure mathematics, the term ``space'' is
omnipresent. It is also subject to ambiguous and sometimes inconsistent
...
implied, structure. Consequently, when we we see the term we have to
make sure we understand the exact intent of the author.
...
cannot be indexed by the integers, we refer to it as an
\textbf{uncountably infinite set}, or simply an \textbf{uncountable
set}. If we want to imply additional structure then we might also say
\textbf{countable space} and \textbf{uncountable space}.

Mathematically, sets are quite abstract. The elements of a set, for
example, can be indivisible objects, but they can also be sets
themselves. At the same time we can construct sets whose elements are
functions that map elements between two other sets.
...
variables that take values in that set. For example, \(x\) will denote a
variable taking values in the set \(X\) or, more compactly, \(x \in X\).
...
\(x, x', x'' \in X\) all denote distinct variables that each correspond
to arbitrary elements in \(X\). We can also use integer indices to
distinguish between different variables when helpful; in this case
\(x_{1}, x_{2}, x_{3} \in X\) would denote three distinct variables. At
the same time, non-numerical labels such as
\(x_{a}, x_{b}, x_{c} \in X\) or even
...
\textbf{bound variables}, respectively. To distinguish between the two,
I will decorate bound variables with a tilde; in words \(x\) denotes any
...
In these cases subsets are more easily defined \emph{implicitly} as the
elements satisfying a certain condition. These implicit definitions are
neatly encapsulated in the \textbf{set-builder notation}, \[
\mathsf{x} = \{ x \in X \mid \text{ condition(x) } \}
\] which we can read as ``the elements of \(X\) such that the
element-wise condition is satisfied''. When there is no ambiguity about
the ambient set, the set-builder notation can be simplified to \[
\mathsf{x} = \{ \text{ condition(x) } \}
\]

If, for example, the real-valued function
\(f : X \rightarrow \mathbb{R}\) assigns to each element \(x\) a value
\(f(x)\), then we could define a subset as the collection of elements
that are assigned the particular value \(0\). In set-builder notation
this would become \[
\mathsf{x} = \{ x \in X \mid f(x) = 0 \}.
\]

Every power set always contains a few special subsets. The \textbf{empty
set} consists of no elements, \textbf{atomic sets} or \textbf{singleton
sets} each consist of a single element, and a \textbf{full set} consists
of the entire set (Figure~\ref{fig-subsets}). Most subsets, however,
contain an intermediate number of elements.

One of the key features of uncountable sets is that most of its subsets
also contain an uncountable number of elements. To visually represent
subsets containing an uncountable number of elements I will use filled
shapes to contrast against individual points.
...
\captionsetup{labelsep=none}\includegraphics{figures/sets/examples/empty/empty.pdf}

}

\subcaption{\label{fig-empty}}
...
\captionsetup{labelsep=none}\includegraphics{figures/sets/examples/atomic/atomic.pdf}

}

\subcaption{\label{fig-atomic}}
...
\captionsetup{labelsep=none}\includegraphics{figures/sets/examples/general/general.pdf}

}

\subcaption{\label{fig-general}}
...
\captionsetup{labelsep=none}\includegraphics{figures/sets/examples/full/full.pdf}
...
\caption{\label{fig-subsets}On a general set we can always construct (a)
an empty set containing no elements, (b) atomic sets containing only a
single element, and (d) a full set containing all elements in the set.
(c) Most subsets are more intermediate, containing more than one but not
all of the elements. Here filled shapes represent a potentially
...
\captionsetup{labelsep=none}\includegraphics{figures/sets/unary_set_operations/input/input.pdf}

}

\subcaption{\label{fig-unary-input}}
...
\captionsetup{labelsep=none}\includegraphics{figures/sets/unary_set_operations/complement/complement.pdf}
...
subset consisting of all elements in the ambient set that are not
contained in the initial subset.}

\end{figure}%

For any set, the complement of the empty set is always the full set and
vice versa, \begin{align*}
\emptyset^{c} &= X
\\
X^{c} &= \emptyset.
\end{align*}

The binary union and intersection operations also generalize to any set
(Figure~\ref{fig-binary}). A \textbf{union} of two input subsets is the
subset containing all of the elements contained in \emph{either} input
subset, \begin{alignat*}{6}
...
\captionsetup{labelsep=none}\includegraphics{figures/sets/binary_set_operations/inputs/inputs.pdf}

}

\subcaption{\label{fig-binary-inputs}}
...
\captionsetup{labelsep=none}\includegraphics{figures/sets/binary_set_operations/union/union.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/sets/binary_set_operations/intersection/intersection.pdf}
...
\caption{\label{fig-binary}The union and intersection operations relate
two input subsets to a single output subset. (a) A union consists of all
elements in either input subset, while (b) an intersection consists of
all elements in both input subsets.}

\end{figure}%

Two subsets are said to be \textbf{disjoint} when they don't overlap,
and hence don't share any elements. More formally, two subsets are
disjoint when their intersection is the empty set, \[
\mathsf{x}_{1} \cap \mathsf{x}_{2} = \emptyset.
\]

All of the properties of these binary operations that we discussed for
finite sets also generalize to arbitrary sets. For instance, the union
and intersection of a subset with itself always returns that subset, \[
...
complement operation. For example, the union of a subset and its
complement is, by construction, the full set, \[
...
A collection of elements alone is only so interesting mathematically,
and only so useful in practice. The sets of theoretical and practical
...
underlying set and the additional structure together. For instance, if
we denote the set \(X\) and the structure \(\mathfrak{s}\) then we would
denote the space that pairs them by \((X, \mathfrak{s})\).
Unfortunately, these additional structures are often implicit and taken
for granted. In particular, \(X\) is often used to denote both the
underlying set \emph{and} the structure-informed space.

In this section we will review just a few of the structures that play
important roles in practical applications. Specifically, we will focus
on the structures that define the most common spaces that arise in
mathematical modeling. We have already encountered \emph{probabilistic}
structures in
\href{http://www.symplectomorphic.com/assets/chapters_html/probability_on_finite_sets.html}{Chapter
One}, and we'll explore them in full generality in
...
element \(x_{1}\) appears earlier in the ordering than \(x_{2}\). In
this case we also say that \(x_{1}\) is \emph{smaller} than \(x_{2}\),
and that \(x_{2}\) is \emph{larger} than \(x_{1}\).
...
Intuitively, we can always interpret a strict ordering as a geometric
arrangement of the elements along a one-dimensional line. Partial
orderings are more general, but they can often be interpreted as a
geometrical arrangement of the elements along a higher-dimensional
plane. Critically, neither geometric interpretation relies on any notion
of distance between the elements; any arrangement does not specify how
far apart any two elements are.

A set equipped with a particular ordering is referred to as both an
\emph{ordered space} and an \emph{ordinal space}. Likewise, a set
equipped with a particular partial ordering is referred to as a
\emph{partially-ordered space}.
...
education that it can be especially easy to take for granted. It
typically refers to equipping a set with one or more \emph{binary
operations} that manipulate any two input elements into a single output
element. Some algebraic structures also introduce \emph{unary
operations} that transform a single input element into a single output
element.
...
with one exception, not usually referred to as ``algebraic spaces''. In
most cases they are not even referred to as ``spaces''! Instead sets
equipped with each algebraic structure are given special names, such as
``magma'' and ``group'' and ``field'', that is not obviously related to
the others.
...
\paragraph{Single Algebraic Operation}\label{single-algebraic-operation}
...
\end{alignat*} where \(x_{1}\) denotes the first input element,
...
For instance, a binary operation that is symmetric with respect to the
input elements, \[
...
chaining multiple evaluations together. For example, we can apply a
binary operation to the three elements \(x_{1}\), \(x_{2}\), and
\(x_{3}\) by first taking \(x_{1}\) as \(x_{2}\) inputs and then taking
the resulting output as an input along with \(x_{3}\), \[
(x_{1} \cdot x_{2}) \cdot x_{3}.
\] Alternatively, we can first take \(x_{2}\) and \(x_{3}\) as inputs
and then take \(x_{1}\) and the resulting output as inputs, \[
x_{1} \cdot (x_{2} \cdot x_{3}).
\] These two groupings will, in general, yield \emph{different} outputs.
Operations that always give the same output for both groupings are
referred to as \textbf{associative} operations.

Some binary operations also distinguish special elements. For instance,
...
Similarly, an operation might allow a particular element to determine
the output no matter what the other input element is. Formally, there
might exist an element \(x_{\text{Null}}\) such that \[
x \cdot x_{\text{Null}}
=
x_{\text{Null}} \cdot x
=
...
between the other elements in the set. Specifically, some elements
\(x \in X\) might be paired with another element \(x^{-1} \in X\) such
that the binary operation returns the distinguished identity element, \[
x \cdot x^{-1} = x^{-1} \cdot x = x_{\text{Id}}.
\] In this case we say that \(x^{-1}\) is the \textbf{inverse element}
of \(x\). By construction, \(x\) is also the inverse element of
\(x^{-1}\) and the identity element is always its own inverse element.
Conversely, because inputing null elements to a binary operation cannot
result in an identity element output, null elements can never have
inverse elements.
...
When working with the integers, no element features a multiplicative
inverse except for the identity element. On the other hand, every real
number \(x\) except for \(x_{\text{Null}} = 0\) features a
multiplicative inverse element known as its \textbf{reciprocal} that is
...
If one algebraic operation is good, then two should certainly be better.
Multiple algebraic operations that are \emph{compatible} with each other
define particularly rich algebraic structures.

In the context of two binary algebraic operations, a common convention
is to write the first operator as \begin{alignat*}{6}
...
distributes across addition. An algebraic structure comprised of these
two compatible operations is also sometimes known as an
...
A \textbf{linear algebra over the real numbers} complements this scalar
multiplication operation with an associative, commutative, and unital
binary operator \begin{alignat*}{6}
+ :\; & X \times X& &\rightarrow& \; & X &
\\
& x_{1}, x_{2} & &\mapsto& & x_{1} + x_{2} &
\end{alignat*} that interacts nicely with \(\cdot\). For example, a
linear algebra assumes that \(\cdot\) distributes over \(+\), \[
...
of \textbf{distance} between individual elements. Mathematically, this
is done by introducing a binary operation that takes two elements as
inputs and returns a positive real number as output, \begin{alignat*}{6}
d :\; & X \times X& &\rightarrow& \; & [0, \infty] &
\\
& x_{1}, x_{2} & &\mapsto& & d(x_{1}, x_{2}) &.
\end{alignat*}

In order for this operation to conform with out intuitive notions of
distance, we need it to satisfy a few key properties. For instance, the
output should vanish when comparing an element to itself, \[
d(x_{1}, x_{2}) = 0
\] whenever \(x_{1} = x_{2}\). At the same time, the output should be
non-zero when comparing two distinct inputs, \[
d(x_{1}, x_{2}) > 0
\] whenever \(x_{1} \ne x_{2}\). The operation should also be symmetric
in its inputs, \[
...
complement the arrangement defined by an ordering: an ordering defines
how elements are arranged but not how far apart they are, while a metric
defines how far apart the elements are but not their arrangement.
Together an ordering and metric define a particularly rigid arrangement
of the elements.
...
\textbf{limit} \(x\) if for \emph{any} distance \(\epsilon > 0\), no
matter how small it might be, we can always go deep enough into the
sequence that the distances between the tail elements \[
\{ x_{i'}, x_{i' + 1}, \ldots, x_{i' + i}, \ldots \}
\] and the limiting element \(x\) are all bounded by \(\epsilon\)
(Figure~\ref{fig-metric-convergence}), \begin{align*}
d(x_{i'}, x) &< \epsilon
\\
d(x_{i' + 1}, x) &< \epsilon
\\
...
\\
\ldots&.
\end{align*} In other words, the elements of a convergent sequence
become arbitrarily close to \(x\), at least eventually.

\begin{figure}

\centering{

\includegraphics[width=0.9\textwidth,height=\textheight]{figures/structures/metric_convergence/metric_convergence.pdf}
...
instead distinguishing certain subsets.
...
An open ball with zero radius reduces to the central element, \[
\mathsf{b}_{0}(x) = \{ 0 \}.
\]
...
The elements exactly a distance of \(r\) away from \(x\) define yet
another subset, \[
...
\includegraphics[width=0.5\textwidth,height=\textheight]{figures/structures/metric_topology/continuous_neighborhoods/continuous_neighborhoods.pdf}
...
relationships between individual elements in a metric space. For
example, because any pair of distinct elements are separated by a finite
distance they can always be contained in non-overlapping metric
neighborhoods (Figure~\ref{fig-metric-separation}). In other words,
metric neighborhoods are able to \emph{distinguish} individual elements
from each other.
...
\includegraphics[width=0.5\textwidth,height=\textheight]{figures/structures/metric_topology/separation/separation.pdf}
...
sequences. Recall that, by definition, we can always go deep enough into
a convergent sequence so that the tail elements \[
\{ x_{i'}, x_{i' + 1}, \ldots, x_{i' + i}, \ldots \}
...
elements are all contained in the open ball \(\mathsf{b}_{\epsilon}(x)\)
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/structures/metric_topology/convergence/convergence.pdf}
...
elements are contained in any metric neighborhood.}

\end{figure}%

Metric neighborhoods can also be used to formalize intuitive notions of
``continuous'' and ``discrete'' metric spaces. For instance, we can
...
words, no metric neighborhood in a continuum can ever reduce to an
atomic set.

On the other hand, the elements in a discrete metric space are not
separated by arbitrary distances. They are instead separated by only
...
degenerate, reducing to the same subset of \(X\)
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/structures/metric_topology/discrete_neighborhoods/discrete_neighborhoods.pdf}
...
same subset. In particular, the smallest metric neighborhoods all
...
The concept of open subsets can also be generalized beyond just open
...
one metric neighborhood that is also contained in \(\mathsf{x}\), we can
always move closer to the edge of the subset without leaving it.
...
subsets} as the complements of open subsets. Closed subsets are
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/structures/metric_topology/open_subset/open_subset.pdf}
...
but never vanish. This allows us to make progressively smaller moves
towards the boundary without ever actually reaching it.}
...
\includegraphics[width=0.5\textwidth,height=\textheight]{figures/structures/metric_topology/closed_subset/closed_subset.pdf}

}

\caption{\label{fig-closed-subset}A closed subset of a metric space
contains boundary elements whose metric neighborhoods all reach outside
of the subset.}

\end{figure}%

Unfortunately, this construction does introduce a few subtleties. Unlike
the colloquial use of the words ``open'' and ``closed'', these
definitions of open subsets and closed subsets are not exact opposites.
If a subset and its complement are both open then they will, by
definition, be both open and closed at the same time. Because
mathematicians have no shame, these subsets are referred to as
``clopen'' subsets. Moreover, open subsets and closed subsets are both
exceptional in the power set; in general only \emph{some} subsets will
be open, closed, or both, and most subsets will be neither open nor
closed.
...
\(\mathsf{x}_{1} \cup \mathsf{x}_{2}\) will also be an open subset. In
fact, the union of \emph{any} number of open subsets will always be
open. Likewise, if \(\mathsf{x}_{1}\) and \(\mathsf{x}_{2}\) are both
open subsets then \(\mathsf{x}_{1} \cap \mathsf{x}_{2}\) will also be an
open subset. Indeed, the intersection of any \emph{finite} number of
open subsets will also be open but, in an unfortunate asymmetry, the
intersection of an infinite number of open subsets might not be open.
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/structures/metric_topology/equivalent_open_subsets/equivalent_open_subsets.pdf}
...
arbitrary unions, and is closed under finite intersections. Any set
equipped with a topology is referred to as a \textbf{topological space}.
...
If the open subsets in a topology are derived from a particular metric,
or \emph{can be} derived from some metric, then we refer to the topology
as a \textbf{metric topology}.
...
\includegraphics[width=0.5\textwidth,height=\textheight]{figures/structures/general_topology/neighborhoods/neighborhoods.pdf}
...
elements in the proximity of \(x\). Smaller subsets define smaller
neighborhoods with stronger notions of proximity.}
...
topologies. In other words, the larger the topology is the closer we can
zoom into the individual elements.

For instance, the smallest topology, known as the \textbf{trivial
...
shared by \emph{all} of the elements in \(X\). Consequently, the
topology is not able to discriminate any element from any other element.
...
in \hyperref[sec:open-balls]{Section 1.2.4.1}.

\begin{figure}

\centering{

\includegraphics[width=0.5\textwidth,height=\textheight]{figures/structures/general_topology/discrete_topology/discrete_topology.pdf}

}

\caption{\label{fig-general-discrete-neighborhoods}The discrete topology
contains all of the atomic sets, each of which defines a neighborhood
that completely isolates an element from every other element in the
ambient set.}
...
by open subsets on metric spaces. These \textbf{Hausdorff topologies}
are, in other words, able to distinguish every element from every other
element.
...
\includegraphics[width=0.5\textwidth,height=\textheight]{figures/structures/general_topology/separation/separation.pdf}
...
\hyperref[sec:open-balls]{Section 1.2.4.1} are generalized by topologies
that fall in between the extreme trivial and discrete topologies. They
contain enough subsets to be Hausdorff, but not so many to be discrete.
This allows us to zoom in arbitrary close to each element, but never
completely isolate them from neighboring elements. The precise subsets
included in these intermediate topologies endow a set a certain notions
of ``shape''.
...
boundary of one. This separates the two ends of the interval from each
other and gives the line its distinctive shape
(Figure~\ref{fig-line-topology}). Removing these subintervals gives
\emph{another} Hausdorff topology, but one where the obstructions
between the elements near the lower boundary and the elements near the
upper boundary have been removed. This effectively connects the two ends
together, endowing the set not with the shape of a line but rather the
shape of a circle (Figure~\ref{fig-circle-topology})!
...
\captionsetup{labelsep=none}\includegraphics{figures/structures/general_topology/line_topology/line_topology.pdf}

}

\subcaption{\label{fig-line-topology}}
...
\captionsetup{labelsep=none}\includegraphics{figures/structures/general_topology/circle_topology/circle_topology.pdf}
...
topology endow the ambient set with subtle notions of shape. (a) A line
is an uncountably infinite set equipped with a topology that contains
certain subsets that separate the ends from each other, shown here in
light red. (b) Removing those particular subsets yields a smaller
topology where the ends are in closer proximity, giving the \emph{same
set} the shape of a circle instead of a line.}

\end{figure}%

Convergence on topological spaces can be defined in the same way as it
is on metric spaces, we just need to replace the neighborhoods derived
from open balls to the neighborhoods defined by a topology
(Figure~\ref{fig-general-convergence}). The generality of topological
structure, however, does come at a cost: if a topology cannot separate
individual elements then the limit of a convergent sequence might not be
unique. Instead unique limits are guaranteed by only Hausdorff
topologies.
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/structures/general_topology/convergence/convergence.pdf}
...
sequence that all subsequent elements are contained in any topological
neighborhood of \(x\). For many topologies, however, a sequence can
converge to multiple elements at the same time. Unique limits are a
feature of only some topologies.}

\end{figure}%

A subtle benefit of this topological definition of convergence is that,
because it doesn't require a metric, it also doesn't require us to
define the real numbers that a metric outputs. This is helpful for
avoiding circular logic when trying to formally define the real numbers
in the first place!
...
mathematics. Specifically, a given topology can be used to quantify
which behaviors will be \emph{shared} by different choices of other
...
\hyperref[sec:open-sets]{Section 1.2.4.2} we saw that different metrics
on the same set can define the same open sets, and hence the same metric
...
that common topology. For instance, on metric spaces with the same
...
explored in \hyperref[sec:general-topology]{Section 1.2.4.3} will share
...
topology of a circle will exhibit the same circular ``shape''. Different
metrics can warp how far apart the elements are from each other, but
they cannot disrupt those topologically-derived shapes.
...
that structure. Consequently, topological structure is often interpreted
as more \emph{fundamental} than other structures. From this perspective,
the foundation of a space is a set equipped with a particular topology.
Any additional structure, such as an ordering, algebra, or metric just
...
\emph{compatible} with each other. For example, if we want to equip a
set with both a topology and a metric then the resulting space will be
particularly well-behaved if we use a metric topology. At the same time,
ambient structure can also distinguish certain compatible subsets.

For example, if a set is equipped with an ordering then we can define
...
open and closed intervals also be topologically open and closed.

As we saw in \hyperref[sec:open-balls]{Section 1.2.4.1}, a metric
distinguishes subsets of elements surrounding a given element. This
includes both open balls, \[
\mathsf{b}_{r}(x)
=
\{ x' \in X \mid d(x, x') < r \}
\] and closed balls, \[
...
complementary closed subsets. If an ordering, metric, and topology are
all consistent with each other then we can associate these topological
subsets with intervals and balls. In general, however, each structure
can distinguish different subsets.

Because every topology contains the empty set and the full set, these
subsets are always of interest. Moreover, when working with discrete
topologies the atomic subsets that contain only a single element are
particularly relevant.

\section{Prototypical Spaces}\label{sec:proto-spaces}

The more structure that we endow onto a set, the richer its properties
will be. That richness offers not only mathematical but also
interpretational opportunities; indeed the spaces that are most common
in practice are absolutely spoiled with structure. In section we'll
review the spaces that arise most often in mathematical modeling.

\subsection{Power Set}\label{power-set}

The subsets of any set automatically feature a surprising amount of
structure. We can then use that structure to elevate the power set into
a space.
...
in the sequential ordering. For any set, the empty set is always the
smallest subset and the full set is always the largest.
...
The identity element for the union operation is empty set, \[
\mathsf{x} \cup \emptyset
=
\emptyset \cup \mathsf{x}
=
...
Similarly, because the intersection of any subset with the empty set is
the empty set, \[
\mathsf{x} \cap \emptyset = \emptyset \cap \mathsf{x} = X,
\] the empty set is an absorbing element of the intersection operation.
...
discrete quantities and indexing sequences of objects. Despite their
ubiquity, however, we often take the structures that define the integers
for granted.
...
can define additive inverses, and hence subtraction, but we cannot
...
integers is the same, \begin{align*}
d(x_{n + 1}, x_{n})
&= | x_{n + 1} - x_{n} |
\\
&= | (x_{n} + 1) - x_{n} |
...
\\
&= 1.
\end{align*} Consequently the space is especially uniform.

Because this metric enforces a minimum distance between every pair of
elements, it is compatible with the discrete topology. In particular,
the atomic sets in the discrete topology maintain a strong separation
...
algebraic structure can define a different metric structure; elements
that start next to each other can be permuted to be arbitrarily far
apart. In general, only the discrete topology is invariant to
permutations.

Despite all of this chaos, however, the behavior of the permuted space
still conforms with our intuitions about the integers. In other words,
we've reached a bit of an existential ambiguity. Every countably
infinite set equipped with the discrete topology and \emph{any} choice
of compatible strict ordering, arithmetic operations, and metric defines
a space which exhibits all of the features we expect of the integers!
...
about \emph{the} integers, but at the same we have to specify a
particular configuration to use the integers in any practical
application.

Regardless of which interpretation we embrace, ``the'' integers admit a
few particularly useful subspaces. We can construct the \textbf{natural
numbers}, \[
...
the integers, but any the inherited algebraic structure will generally
be modified. For instance, we can't consistently define subtraction on
the natural numbers because the difference between two positive integers
can be negative, which falls outside of \(\mathbb{N}\).
...
\(\mathbb{R}\). This space is also known as the \textbf{real numbers}
and a \textbf{one-dimensional Euclidean space}. The real line sets the
stage for important constructions like differential and integral
calculus, but it also exhibits some subtle properties that can
compromise our intuitions and require a bit of careful technical detail
at times to avoid inconsistent behavior.
...
\textbf{flexible real line} interpretation. In this latter perspective,
...
use to construct useful subspaces. For example, the integers are a
subset of the real line, \(\mathbb{Z} \subset \mathbb{R}\). We can also
define the \textbf{positive real line} as \[
\mathbb{R}^{+} = \{ x \in \mathbb{R} \mid x \ge 0 \}.
\] More generally, any interval of a real line defines a subspace. All
of these subspaces inherit topological, ordering, and metric structure
from the initial real line, but they may not inherit all of the
algebraic structure.
...
\includegraphics[width=0.5\textwidth,height=\textheight]{figures/real_line_grid/real_line_grid.pdf}
...
infinities themselves. An \textbf{extended real line} introduces two new
elements: one smaller than all elements in the initial real line to
represent negative infinity, and one larger than all of the elements to
represent positive infinity. In other words, we can interpret an
extended real line as the closed interval \([-\infty, \infty]\) while a
real line is the corresponding open interval \((-\infty, \infty)\).

The inclusion of these two points does introduce some slight technical
challenges. For instance, the algebraic structure is complicated by the
...
\href{http://www.symplectomorphic.com/assets/chapters_html/probability_on_finite_spaces.html}{Chapter
One}, where we used it to model the potentially infinite reservoir that
we want to allocate across a finite set. In
\href{http://www.symplectomorphic.com/assets/chapters_html/probability_on_general_spaces.html}{Chapter
Four} we will use it model a potentially infinite reservoir to be
...
elements in different sets but also any structures that we have endowed
onto those sets.
...
(Figure~\ref{fig-function}). We denote functions as
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/maps/function/function.pdf}
...
elements of given output set. Some relations, however, are more useful
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/injective/injective.pdf}

}

\subcaption{\label{fig-injective}}
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/non_injective/non_injective.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/surjective/surjective.pdf}

}

\subcaption{\label{fig-surjective}}
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/non_surjective/non_surjective.pdf}
...
When a function is both injective and surjective, \emph{every} output
element is uniquely paired with a single input element. In other words,
the input set and the output set are in perfect correspondence. These
functions are known as \textbf{bijective} functions.
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/graphs/f1/f1.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/graphs/f2/f2.pdf}

}

\subcaption{\label{fig-f2}}
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/graphs/f3/f3.pdf}

}

\subcaption{\label{fig-f3}}
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/graphs/f4/f4.pdf}
...
\(y = \frac{1}{2}\), and hence is not surjective. On the other hand,
each distinct input element maps to a distinct output element so, unlike
the function \(f_1\), the function \(f_2\) is injective.

This function (Figure~\ref{fig-f3}) \begin{alignat*}{6}
f_3 :\; & [0, 1] & &\rightarrow& \; & [0, 1] &
\\
& x & &\mapsto& & x \cdot (1 - x) &
\end{alignat*} shares the same symmetry as \(f_1\) -- both \(x\) and
\(1 - x\) map to the same output element, making the function
non-injective. Unlike \(f_1\), however, the output elements of \(f_3\)
completely fill up the output set, making \(f_3\) a surjective function.
...
are known as \textbf{automorphisms}. A bijective automorphism doesn't
change \(X\), but may permute its elements around. This permutation can
in turn warp any structure endowed on \(X\).
...
their action and recover input elements. Formally, if
\(f : X \rightarrow Y\) is a bijection then we can define an
\textbf{inverse function} \(f^{-1} : Y \rightarrow X\) such that \[
f^{-1}(f(x)) = x
\] for all \(x \in X\). Because no information is lost when we map from
\(X\) to \(Y\), we can completely recover \(X\) again if needed
(Figure~\ref{fig-function-inverse}).

\begin{figure}

\centering{

\includegraphics[width=0.9\textwidth,height=\textheight]{figures/maps/inverse/inverse.pdf}

}

\caption{\label{fig-function-inverse}A bijective function \(f\)
identifies every output element with one and only input element.
Consequently, we can construct an inverse function that takes each
output element back to the corresponding input element.}

\end{figure}%

As with the term ``identity'' we have to be careful when using the term
``inverse'' in the context of algebraic spaces. Specifically, we have to
...
distinct output elements to distinct input elements. If a function is
not surjective, then some output elements might not be related to
\emph{any} input elements. On the other hand, if a function is not
injective then some output elements may be related to \emph{multiple}
input elements.

Although we cannot generally relate elements \(y \in Y\) to unique
elements of \(X\), we can relate them to the \emph{subsets} of \(X\)
that contain all of the elements that map into \(y\), \[
f^{-1}(y) = \{ x \in X \mid f(x) = y \} \subset X.
\] The subset \(f^{-1}(y)\) is referred to as the \emph{level set} or
\emph{fiber} of \(y\). Intuitively, we can reconstruct the entire input
set by ``weaving'' these fibers together, \[
X = \cup_{y \in Y} f^{-1}(y).
\]

If \(f\) is not surjective then some level sets will be empty, but if
\(f\) is surjective then all level sets will be non-empty. At the same
time, the level sets of non-injective functions will generally contain
multiple elements while the level sets of injective functions will
contain a single element or no elements. The level sets of bijective
functions contain one and only one element: the result of evaluating the
inverse function.

Level sets allow us to define a generalized notion of inversion for
arbitrary functions, where we map output elements to their corresponding
level set (Figure~\ref{fig-fibers}), \begin{alignat*}{6}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/maps/fibers/fibers.pdf}
...
to a level set, or fiber, of input elements \(f^{-1}(y) \subset X\). In
...
When formalizing the defining property of an inverse function, we took
the output of a function \(y = f(x)\) and used it as input to the
inverse function, \(f^{-1}(y) = f^{-1}(f(x))\). This chaining of
functions with compatible input and output sets is known as
\textbf{function composition}, or just \textbf{composition} for short.
...
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/maps/composition/composition.pdf}

}

\caption{\label{fig-composition}The functions \(f : X \rightarrow Y\)
and \(g : Y \rightarrow Z\) can be chained together, mapping an input
element \(x \in X\) to an intermediate element \(y \in Y\) which is then
mapped to an output element \(z \in Z\). This combined mapping defines
the composite function \(g \circ f\).}

\end{figure}%

Using this notation, we can define an inverse function a bit more
compactly as the unique function satisfying the composition condition \[
\text{Id} = f^{-1} \circ f.
\] In words, the composition of a bijective function with its inverse
function must be the identify function.
...
choose that structure arbitrarily, or we can try to derive it from the
input structure \(\mathfrak{x}\).

Fortunately, maps between sets often \emph{induce} natural maps between
structures. This allows us to fully transform spaces without having to
make additional assumptions about associated structures. Unfortunately,
these induced maps don't always give the structure that we might expect.
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/set_pushforward/set_pushforward.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/set_pullback/set_pullback.pdf}
...
This pattern is not exceptional. Sufficiently well-behaved functions
that map an input set into an output set often induce mappings from
objects defined over that input set to objects defined over that output
set or vice versa. This includes not only subsets but also all of the
structures that elevate a set into a space that we introduced in
\hyperref[sec:structures]{Section 1.2}.

To be a bit more formal, let \(X\) be the set defining an input space
and \(\mathfrak{x}\) the particular structure, or structures, that
elevates that set into a space; in this case, we can completely specify
our space as \((X, \mathfrak{x})\). Moreover, denote the set of all
structures that we can endow onto the input space \(X\) as
\(\mathfrak{X}\), and the set of all structures that we can endow onto
the output space \(Y\) as \(\mathfrak{Y}\).
...
compatible output structures, \begin{alignat*}{6}
...
function on a given input structure, \(f_{*}(\mathfrak{x})\). If there
might be any ambiguity then we have to be careful to use ``pushforward''
as an adjective, explicitly modifying ``function'' or ``structure'', and
not an ambiguous noun.

Similarly, the subset map \(f^{*} : 2^Y \rightarrow 2^X\) that we
constructed above \emph{pulls} output subsets \emph{back} against the
action of the function \(f\). More generally, any relation induced by
\(f : X \rightarrow Y\) that pulls output structures back into
compatible input structures, \begin{alignat*}{6}
...
structure. In general, the nicer the function \(f\) is the more
structure-transforming functions it induces.

\paragraph{Relating Orderings}\label{relating-orderings}

A function \(f : X \rightarrow Y\) allows us to implicitly compare two
input elements by explicitly comparing their output values. In other
words, we can pull back output comparisons to input comparisons.

Consider, for example, orderings. If the output space \(Y\) is equipped
with a partial ordering \(<_{Y}\), then for any function \(f\) we can
define a partial ordering on the input space as \[
x_{1} \;\; (f^{*}\!\!<_{Y}) \; x_{2}
\] if and only if \[
f(x_{1}) <_{Y} f(x_{2}).
\]

Pulling back strict orderings requires a bit more care. In general,
distinct input elements can map to the same output element, \[
f(x_{1}) = f(x_{2}),
...
comparisons still define a perfectly well-behaved \emph{partial}
ordering on the input set.

To review, an arbitrary function \(f : X \rightarrow Y\) pulls back
\emph{any} ordering on the output space to a partial ordering on the
input space. Injective functions in particular also pull back strict
orderings on the output space to strict orderings on the input space.

Pushing forward orderings from the input space to the output space is
not quite as easy as pulling back orderings. If any pair of output
elements are associated with multiple input elements, \begin{align*}
y_{1} &= f(x_1) = f(x_1')
\\
y_{2} &= f(x_2) = f(x_2'),
...
\\
x_2' &< x_1',
\end{align*} that cannot be reconciled into a consistent output
ordering.

Consequently, we can push orderings on the input space forward to
partial orderings on the output space only if \(f\) is surjective. If we
want to push strict orderings on the input space forward to strict
orderings on the output space, then we also need \(f\) to be surjective,
and hence bijective.
...
and returns a single element. In order to pushforward an operation
\(\cdot\) on the input space, we would need to map output elements to
input elements, apply the operation, and then map the result back to the
output space, \[
y_1 \; (f_{*} \cdot) \; y_2
\equiv
...
Similarly, in order to pullback an operation \(\cdot\) on the output
space, we would need to map input elements to output elements, apply the
operation, and then map the result back to the input space, \[
x_1 \; (f^{*} \cdot) \; x_2
\equiv
...
In particular, if \(f\) is not injective then \(d(f(x_1), f(x_2))\) can
vanish even if \(x_1 \ne x_2\). Fortunately, if \(f\) is injective then
\((f^{*} d)(x_1, x_2)\) will satisfy all of the properties of a metric
on the input set. In this case we we denote it the \textbf{pullback
metric}.
...
This output collection, however, will not always satisfy the properties
of a topology. For example, if \(f\) it not surjective then the output
collection will not contain the full set as needed. Even if \(f\) is
surjective, the pushforward sets are not guaranteed to be closed under
intersections and hence will not, in general, define a topology.

On the other hand, pullbacks sets are a bit more well-behaved. Pulling
back the subsets that define a topology on the output set, \[
\mathfrak{t}_{Y}
=
...
information about the individual elements or any structure endowed to
those elements!
...
\((Y, f_{*}(\mathfrak{x}))\). Similarly, for any structure that can be
pulled back against \(f\), any output space \((Y, \mathfrak{y})\)
automatically defines a compatible input space
\((X, f^{*}(\mathfrak{y}))\).
...
For instance, consider a strictly ordered input space \((X, <{X})\) and
a strictly ordered output space \((Y, <{Y})\). Any injective function
\(f: X \rightarrow Y\) defines a pullback strict ordering
\(f^{*} \!\! <{Y}\) on the input space by \[
x_{1} \;\; (f^{*}\!\!<_{Y}) \; x_{2}
\] if and only if \[
f(x_{1}) <_{Y} f(x_{2}).
\] There's no guarantee, however, that this will be consistent with the
initial ordering \(<{X}\). Instead, we will have \(f_{*} <{Y} = <{X}\)
only for the exceptional functions satisfying \[
f(x_{1}) <{Y} f(x_{2})
\] if and only if \[
x_{1} <{X} x_{2}.
\] These injective functions are known as \textbf{monotonically
increasing functions}. Injective functions that \emph{flip} the
orientation of the orderings, so that \[
f(x_{1}) <{Y} f(x_{2})
\] if and only if \[
x_{2} <{X} x_{1}
\] are known as \textbf{monotonically decreasing functions}.

More generally, functions that respect a \emph{partial} ordering on the
input space and any ordering on the output space are known as
\textbf{monotonically non-decreasing functions}. Those that reverse the
orientation of the partial orderings are known as \textbf{monotonically
...
Consequently, \(f_1\) is monotonically increasing but \(f_2\) is not
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/monotonic/monotonic.pdf}

}

\subcaption{\label{fig-monotonic}}
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/non_monotonic/non_monotonic.pdf}
...
preserve orderings so that larger inputs always give larger outputs. The
function (a) \(f_{1} : x \mapsto x^{3}\) is monotonic but the function
(b) \(f_{2} : x \mapsto -x^{2}\) is not.}
...
\] the function \(f\) \emph{is} a homomorphism for the multiplication
operation.
...
\\
& x & &\mapsto& & x - \frac{1}{4} &
\end{alignat*} is an isometry for the natural metric on a real line,
\begin{align*}
d_{Y}(f(x_1), f(x_2))
&=
\sqrt{
...
a warped grid (Figure~\ref{fig-non-isometric}).
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/isometric_pushforward/isometric_pushforward.pdf}

}

\subcaption{\label{fig-isometric}}
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/metric_pushforward/metric_pushforward.pdf}
...
More formally, a bijective function is a homeomorphism if every subset
in the input topology pushes forward to a subset in the output topology
and vice versa. In other words, neighborhoods are preserved and input
elements in proximity to each other are mapped into output elements that
are also in proximity to each other, preserving notions of continuity of
the space. Consequently, homeomorphisms are also known as invertible
\textbf{continuous} functions.

In \hyperref[sec:general-topology]{Section 1.2.4.3}, we saw that lines
and circles can be constructed from a common set equipped with different
...
\includegraphics[width=0.6\textwidth,height=\textheight]{figures/maps/non_homeomorphism/non_homeomorphism.pdf}
...
spaces that we encountered in \hyperref[sec:proto-spaces]{Section 2}
also implies an ambiguity in how we interpret transformations between
them.
...
into another, but only the exceptional monotonically-increasing,
homomorphic, and isometric homeomorphisms will map one of these rigid
real lines into itself.

Conversely, we can take the flexible real line perspective where we
treat the real line as a flexible space defined by a fixed set and
topology but varying orderings, algebras, and metrics. From this
perspective, homeomorphisms maintain the defining structure but
generally transform the auxiliary structures. In other words, a
homeomorphism will in general map one configuration of the flexible real
line into another, \emph{reconfiguring} the flexible real line. Because
configurations are also known as parameterizations, the homeomorphic
transformations are also known as \textbf{reparameterizations}. The
...
While this interpretational ambiguity is often taken for granted, it
have practical consequences. For example, we can interpret
transformations of a physical system as \emph{active} or \emph{passive}.
An active transformation is interpreted as changing the physical system,
while a passive transformation is interpreted as changing how we
represent a fixed physical system. These two views parallel the rigid
and flexible real line perspectives, respectively. Both interpretations
yield equivalent results, but one can be more natural than the other in
some contexts.

Consider, for instance, using a real-valued variable to quantify some
property of a physical system. In this case transforming from one real
line to another can be interpreted in one of two ways. An active
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/rulers/initial/initial.pdf}

}

\subcaption{\label{fig-initial}}
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/rulers/transformed_system/transformed_system.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/maps/rulers/transformed_ruler/transformed_ruler.pdf}
...
physical system remains fixed. Active transformations are well-aligned
with the rigid real line perspective, while passive transformations
well-aligned with the flexible real line perspective.}
...
Similarly, we have the freedom to interpret computer programs written in
different programming languages as either distinct programs or different
implementations of some fixed program with universal inputs and outputs.
In the former interpretation \emph{transpilers} map from one program to
another, but in the latter they map from one implementation to another.

These interpretations are equivalent, and we are free to adopt the one
that is most useful in any given application. In this book I will tend
to default to the flexible real line perspective, where the real line
can be configured infinitely many ways.
...
an underlying set to the many structures that can endow that set with
useful properties, and their transformations. Along the way, we took a
careful look at familiar concepts like variables while also digging into
some more complicated concepts like topology, taking care to appreciate
...
technical resources. At the same time, more technical resources often
...
chapters of Lee (2011) and the appendices of Lee (2013). Similarly, I
find that Curtis (1993) provides an accessible introduction to the more
technical aspects of linear algebra. An additional benefit of these
resources is that they can help acclimate one to the writing style
particular to theoretical mathematics, and make other resources a bit
more accessible.
...
Noll, Pietro Monticone, Tatsuo Okubo, Léo Burgund, and Francesco Terenzi
