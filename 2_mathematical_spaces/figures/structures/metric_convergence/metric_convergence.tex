\documentclass{standalone}
\usepackage{graphicx}	
\usepackage{amssymb, amsmath, amsthm}
\usepackage{color}

\usepackage{tikz}
\usetikzlibrary{math, arrows.meta}

\definecolor{light}{RGB}{220, 188, 188}
\definecolor{mid}{RGB}{185, 124, 124}
\definecolor{dark}{RGB}{143, 39, 39}
\definecolor{highlight}{RGB}{180, 31, 180}
\definecolor{lightteal}{RGB}{186, 219, 219}
\definecolor{midteal}{RGB}{124, 186, 186}
\definecolor{darkteal}{RGB}{38, 142, 142}
\definecolor{darkerteal}{RGB}{0, 124, 124}
\definecolor{darkolive}{RGB}{97, 123, 45}
\definecolor{gray10}{gray}{0.1}
\definecolor{gray20}{gray}{0.2}
\definecolor{gray30}{gray}{0.3}
\definecolor{gray40}{gray}{0.4}
\definecolor{gray60}{gray}{0.6}
\definecolor{gray70}{gray}{0.7}
\definecolor{gray80}{gray}{0.8}
\definecolor{gray90}{gray}{0.9}
\definecolor{gray95}{gray}{0.95}

\begin{document}

\begin{tikzpicture}[scale=1.0]
  
  \pgfmathsetmacro{\m}{7}
  \pgfmathsetmacro{\eps}{1.95}
  
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-3.5, -3) rectangle (3.5, 2.75);
  
    \draw[rounded corners=3, color=black, line width=1] (-3, -2) rectangle (3, 2);
    \node at (0, -2.5) { $X$ };
  
    \foreach [count=\n] \x/\y in {-2.177/-1.092, -2.310/-0.325, -2.207/0.368, -1.858/0.955, -1.178/1.463, 
                                  -0.891/1.780, -0.295/1.687, 0.244/1.431, 0.651/1.105, 1.066/1.167, 
                                  1.393/0.721, 0.971/0.228, 1.260/-0.140, 1.056/-0.296, 0.503/-0.760, 
                                  0.394/-0.874, 0.095/-0.908, -0.132/-0.901, -0.266/-0.434, -0.447/-0.503, 
                                  -0.553/-0.235, -0.304/-0.147, -0.488/0.065, -0.402/0.322, -0.201/0.373} {
      \ifnum \n>\m
        \colorlet{pointcolor}{darkteal}; 
      \else
        \colorlet{pointcolor}{lightteal};
      \fi
      
      \ifnum \n>1
        \draw[color=lightteal, -{Stealth[length=3, width=4]}, 
              line width=1, shorten >=1.25] (A) -- (\x, \y);
        \fill[color=pointcolor] (A) circle (0.05);
      \fi
    
      \ifnum \n=25
        \fill[color=pointcolor] (\x, \y) circle (0.05);
      \fi
      \coordinate (A) at (\x, \y);
    }
    
    \fill[darkteal] (0, 0) circle (0.05) node[right] { $x$ };
  \end{scope} 

  \begin{scope}[shift={(7, 0)}]
    \draw[white] (-3.5, -3) rectangle (3.5, 2.75);

    \draw[gray90, dashed, line width=1] (-2.5, {1.4 * \eps - 2}) -- (2.65, {1.4 * \eps - 2});
    \draw[gray90, dashed, line width=1] ({\m * 5.25 / 26 - 2.5}, -2) -- ({\m * 5.25 / 26 - 2.5}, {1.4 * \eps - 2});
    \fill[gray90] ({\m * 5.25 / 26 - 2.5}, -2.015) rectangle (2.65, {1.4 * \eps - 2});
    \node at (3, {1.4 * \eps - 2}) { $\epsilon_{1}$ };
  
    \draw[->, >=stealth, line width=1.25] (-2.5, -2.015) -- +(0, 4);
    \draw[->, >=stealth, line width=1.25] (-2.515, -2.00) -- +(5.25, 0);
    
    \node[rotate=90] at (-2.95, 0) { $d(x_{i}, x)$ };
    \node at (0, -2.4) { $i$ };
    
    \foreach [count=\n] \x/\y in {-2.177/-1.092, -2.310/-0.325, -2.207/0.368, -1.858/0.955, -1.178/1.463, 
                                  -0.891/1.780, -0.295/1.687, 0.244/1.431, 0.651/1.105, 1.066/1.167, 
                                  1.393/0.721, 0.971/0.228, 1.260/-0.140, 1.056/-0.296, 0.503/-0.760, 
                                  0.394/-0.874, 0.095/-0.908, -0.132/-0.901, -0.266/-0.434, -0.447/-0.503, 
                                  -0.553/-0.235, -0.304/-0.147, -0.488/0.065, -0.402/0.322, -0.201/0.373} {
      \ifnum \n<\m
        \colorlet{pointcolor}{lightteal}; 
      \else
        \colorlet{pointcolor}{darkteal};
      \fi
      \pgfmathsetmacro{\r}{sqrt(\x * \x + \y * \y)}
      \fill[color=pointcolor] ({\n * 5.25 / 26 - 2.5}, {1.4 * \r - 2}) circle (0.05);
    }
    
  \end{scope} 
    
  \pgfmathsetmacro{\m}{14}
  \pgfmathsetmacro{\eps}{1.15}
  
  \begin{scope}[shift={(0, -5.75)}]
    \draw[white] (-3.5, -3) rectangle (3.5, 2.75);
  
    \draw[rounded corners=3, color=black, line width=1] (-3, -2) rectangle (3, 2);
    \node at (0, -2.5) { $X$ };
  
    \foreach [count=\n] \x/\y in {-2.177/-1.092, -2.310/-0.325, -2.207/0.368, -1.858/0.955, -1.178/1.463, 
                                  -0.891/1.780, -0.295/1.687, 0.244/1.431, 0.651/1.105, 1.066/1.167, 
                                  1.393/0.721, 0.971/0.228, 1.260/-0.140, 1.056/-0.296, 0.503/-0.760, 
                                  0.394/-0.874, 0.095/-0.908, -0.132/-0.901, -0.266/-0.434, -0.447/-0.503, 
                                  -0.553/-0.235, -0.304/-0.147, -0.488/0.065, -0.402/0.322, -0.201/0.373} {
      \ifnum \n>\m
        \colorlet{pointcolor}{darkteal}; 
      \else
        \colorlet{pointcolor}{lightteal};
      \fi
      
      \ifnum \n>1
        \draw[color=lightteal, -{Stealth[length=3, width=4]}, 
              line width=1, shorten >=1.25] (A) -- (\x, \y);
        \fill[color=pointcolor] (A) circle (0.05);
      \fi
    
      \ifnum \n=25
        \fill[color=pointcolor] (\x, \y) circle (0.05);
      \fi
      \coordinate (A) at (\x, \y);
    }
    
    \fill[darkteal] (0, 0) circle (0.05) node[right] { $x$ };
  \end{scope} 

  \begin{scope}[shift={(7, -5.75)}]
    \draw[white] (-3.5, -3) rectangle (3.5, 2.75);

    \draw[gray90, dashed, line width=1] (-2.5, {1.4 * 1.95 - 2}) -- (2.65, {1.4 * 1.95 - 2});
    \draw[gray90, dashed, line width=1] (-2.5, {1.4 * \eps - 2}) -- (2.65, {1.4 * \eps - 2});
    \draw[gray90, dashed, line width=1] ({\m * 5.25 / 26 - 2.5}, -2) -- ({\m * 5.25 / 26 - 2.5}, {1.4 * \eps - 2});
    \fill[gray90] ({\m * 5.25 / 26 - 2.5}, -2.015) rectangle (2.65, {1.4 * \eps - 2});
    \node at (3, {1.4 * \eps - 2}) { $\epsilon_{2}$ };
  
    \draw[->, >=stealth, line width=1.25] (-2.5, -2.015) -- +(0, 4);
    \draw[->, >=stealth, line width=1.25] (-2.515, -2.00) -- +(5.25, 0);
    
    \node[rotate=90] at (-2.95, 0) { $d(x_{i}, x)$ };
    \node at (0, -2.4) { $i$ };
    
    \foreach [count=\n] \x/\y in {-2.177/-1.092, -2.310/-0.325, -2.207/0.368, -1.858/0.955, -1.178/1.463, 
                                  -0.891/1.780, -0.295/1.687, 0.244/1.431, 0.651/1.105, 1.066/1.167, 
                                  1.393/0.721, 0.971/0.228, 1.260/-0.140, 1.056/-0.296, 0.503/-0.760, 
                                  0.394/-0.874, 0.095/-0.908, -0.132/-0.901, -0.266/-0.434, -0.447/-0.503, 
                                  -0.553/-0.235, -0.304/-0.147, -0.488/0.065, -0.402/0.322, -0.201/0.373} {
      \ifnum \n<\m
        \colorlet{pointcolor}{lightteal}; 
      \else
        \colorlet{pointcolor}{darkteal};
      \fi
      \pgfmathsetmacro{\r}{sqrt(\x * \x + \y * \y)}
      \fill[color=pointcolor] ({\n * 5.25 / 26 - 2.5}, {1.4 * \r - 2}) circle (0.05);
    }
    
  \end{scope} 
  
  \pgfmathsetmacro{\m}{22}
  \pgfmathsetmacro{\eps}{0.55}
  
  \begin{scope}[shift={(0, -11.5)}]
    \draw[white] (-3.5, -3) rectangle (3.5, 2.75);
  
    \draw[rounded corners=3, color=black, line width=1] (-3, -2) rectangle (3, 2);
    \node at (0, -2.5) { $X$ };
  
    \foreach [count=\n] \x/\y in {-2.177/-1.092, -2.310/-0.325, -2.207/0.368, -1.858/0.955, -1.178/1.463, 
                                  -0.891/1.780, -0.295/1.687, 0.244/1.431, 0.651/1.105, 1.066/1.167, 
                                  1.393/0.721, 0.971/0.228, 1.260/-0.140, 1.056/-0.296, 0.503/-0.760, 
                                  0.394/-0.874, 0.095/-0.908, -0.132/-0.901, -0.266/-0.434, -0.447/-0.503, 
                                  -0.553/-0.235, -0.304/-0.147, -0.488/0.065, -0.402/0.322, -0.201/0.373} {
      \ifnum \n>\m
        \colorlet{pointcolor}{darkteal}; 
      \else
        \colorlet{pointcolor}{lightteal};
      \fi
      
      \ifnum \n>1
        \draw[color=lightteal, -{Stealth[length=3, width=4]}, 
              line width=1, shorten >=1.25] (A) -- (\x, \y);
        \fill[color=pointcolor] (A) circle (0.05);
      \fi
    
      \ifnum \n=25
        \fill[color=pointcolor] (\x, \y) circle (0.05);
      \fi
      \coordinate (A) at (\x, \y);
    }
    
    \fill[darkteal] (0, 0) circle (0.05) node[right] { $x$ };
  \end{scope} 

  \begin{scope}[shift={(7, -11.5)}]
    \draw[white] (-3.5, -3) rectangle (3.5, 2.75);

    \draw[gray90, dashed, line width=1] (-2.5, {1.4 * 1.95 - 2}) -- (2.65, {1.4 * 1.95 - 2});
    \draw[gray90, dashed, line width=1] (-2.5, {1.4 * 1.15 - 2}) -- (2.65, {1.4 * 1.15 - 2});
    \draw[gray90, dashed, line width=1] (-2.5, {1.4 * \eps - 2}) -- (2.65, {1.4 * \eps - 2});
    \draw[gray90, dashed, line width=1] ({\m * 5.25 / 26 - 2.5}, -2) -- ({\m * 5.25 / 26 - 2.5}, {1.4 * \eps - 2});
    \fill[gray95] ({\m * 5.25 / 26 - 2.5}, -2.015) rectangle (2.65, {1.4 * \eps - 2});
    \node at (3, {1.4 * \eps - 2}) { $\epsilon_{3}$ };
  
    \draw[->, >=stealth, line width=1.25] (-2.5, -2.015) -- +(0, 4);
    \draw[->, >=stealth, line width=1.25] (-2.515, -2.00) -- +(5.25, 0);
    
    \node[rotate=90] at (-2.95, 0) { $d(x_{i}, x)$ };
    \node at (0, -2.4) { $i$ };
    
    \foreach [count=\n] \x/\y in {-2.177/-1.092, -2.310/-0.325, -2.207/0.368, -1.858/0.955, -1.178/1.463, 
                                  -0.891/1.780, -0.295/1.687, 0.244/1.431, 0.651/1.105, 1.066/1.167, 
                                  1.393/0.721, 0.971/0.228, 1.260/-0.140, 1.056/-0.296, 0.503/-0.760, 
                                  0.394/-0.874, 0.095/-0.908, -0.132/-0.901, -0.266/-0.434, -0.447/-0.503, 
                                  -0.553/-0.235, -0.304/-0.147, -0.488/0.065, -0.402/0.322, -0.201/0.373} {
      \ifnum \n<\m
        \colorlet{pointcolor}{lightteal}; 
      \else
        \colorlet{pointcolor}{darkteal};
      \fi
      \pgfmathsetmacro{\r}{sqrt(\x * \x + \y * \y)}
      \fill[color=pointcolor] ({\n * 5.25 / 26 - 2.5}, {1.4 * \r - 2}) circle (0.05);
    }
    
  \end{scope} 
    
\end{tikzpicture}

\end{document}  