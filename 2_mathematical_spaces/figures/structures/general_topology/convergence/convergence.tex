\documentclass{standalone}
\usepackage{graphicx}	
\usepackage{amssymb, amsmath, amsthm}
\usepackage{color}

\usepackage{tikz}
\usetikzlibrary{math, arrows.meta}

\definecolor{light}{RGB}{220, 188, 188}
\definecolor{mid}{RGB}{185, 124, 124}
\definecolor{dark}{RGB}{143, 39, 39}
\definecolor{highlight}{RGB}{180, 31, 180}
\definecolor{lightteal}{RGB}{186, 219, 219}
\definecolor{midteal}{RGB}{124, 186, 186}
\definecolor{darkteal}{RGB}{38, 142, 142}
\definecolor{darkerteal}{RGB}{0, 124, 124}
\definecolor{darkolive}{RGB}{97, 123, 45}
\definecolor{gray10}{gray}{0.1}
\definecolor{gray20}{gray}{0.2}
\definecolor{gray30}{gray}{0.3}
\definecolor{gray40}{gray}{0.4}
\definecolor{gray60}{gray}{0.6}
\definecolor{gray70}{gray}{0.7}
\definecolor{gray80}{gray}{0.8}
\definecolor{gray90}{gray}{0.9}
\definecolor{gray95}{gray}{0.95}

% #1: x0
% #2: y0
% #3: R
\newcommand{\randpoints}[3]{
  ({#1 + (1 + 0.2 * rand) * #3 * (-1)},      {#2 + (1 + 0.2 * rand) * #3 * (0)})
  ({#1 + (1 + 0.2 * rand) * #3 * (-0.7071)}, {#2 + (1 + 0.2 * rand) * #3 * (+0.7071)})
  ({#1 + (1 + 0.2 * rand) * #3 * (0)},       {#2 + (1 + 0.2 * rand) * #3 * (+1)})
  ({#1 + (1 + 0.2 * rand) * #3 * (+0.7071)}, {#2 + (1 + 0.2 * rand) * #3 * (+0.7071)})
  ({#1 + (1 + 0.2 * rand) * #3 * (+1)},      {#2 + (1 + 0.2 * rand) * #3 * (0)})
  ({#1 + (1 + 0.2 * rand) * #3 * (+0.7071)}, {#2 + (1 + 0.2 * rand) * #3 * (-0.7071)})
  ({#1 + (1 + 0.2 * rand) * #3 * (0)},       {#2 + (1 + 0.2 * rand) * #3 * (-1)})
  ({#1 + (1 + 0.2 * rand) * #3 * (-0.7071)}, {#2 + (1 + 0.2 * rand) * #3 * (-0.7071)})
}

\begin{document}

\begin{tikzpicture}[scale=0.9]
  
  \pgfmathsetmacro{\m}{7}
  \pgfmathsetmacro{\eps}{1.95}
   
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-3.5, -3) rectangle (3.5, 2.75);
  
    \draw[rounded corners=3, color=black, line width=1] (-3, -2) rectangle (3, 2);
    \node at (0, -2.5) { $X$ };

    \pgfmathsetseed{10}

    \filldraw[darkteal, dashed, line width=1] 
      plot [smooth cycle, tension=0.75] coordinates { \randpoints{0}{-0.15}{\eps} } -- cycle;
    \node[darkteal] at (1.7, -1.6) { $\mathsf{x}_{1}$ };
  
    \foreach [count=\n] \x/\y in {-2.177/-1.092, -2.310/-0.325, -2.207/0.368, -1.858/0.955, -1.178/1.463, 
                                  -0.891/1.780, -0.295/1.687, 0.244/1.431, 0.651/1.105, 1.066/1.167, 
                                  1.393/0.721, 0.971/0.228, 1.260/-0.140, 1.056/-0.296, 0.503/-0.760, 
                                  0.394/-0.874, 0.095/-0.908, -0.132/-0.901, -0.266/-0.434, -0.447/-0.503, 
                                  -0.553/-0.235, -0.304/-0.147, -0.488/0.065, -0.402/0.322, -0.201/0.373} {
      \ifnum \n>\m
        \colorlet{pointcolor}{lightteal}; 
      \else
        \colorlet{pointcolor}{midteal};
      \fi
      
      \ifnum \n>1
        \draw[color=midteal, -{Stealth[length=3, width=4]}, 
              line width=1, shorten >=1.25] (A) -- (\x, \y);
        \fill[color=pointcolor] (A) circle (0.05);
      \fi
    
      \ifnum \n=25
        \fill[color=pointcolor] (\x, \y) circle (0.05);
      \fi
      \coordinate (A) at (\x, \y);
    }
    
    \fill[lightteal] (0, 0) circle (0.05) node[right] { $x$ };
  \end{scope}
    
  \pgfmathsetmacro{\m}{14}
  \pgfmathsetmacro{\eps}{1.15}
  
  \begin{scope}[shift={(7, 0)}]
    \draw[white] (-3.5, -3) rectangle (3.5, 2.75);
  
    \draw[rounded corners=3, color=black, line width=1] (-3, -2) rectangle (3, 2);
    \node at (0, -2.5) { $X$ };
    
    \pgfmathsetseed{10}
    \draw[gray80, dashed, line width=1] 
      plot [smooth cycle, tension=0.75] coordinates { \randpoints{0}{-0.15}{1.95} } -- cycle;
      
    \filldraw[darkteal, dashed, line width=1] 
      plot [smooth cycle, tension=0.75] coordinates { \randpoints{0.05}{0}{\eps} } -- cycle;
    \node[darkteal] at (1.05, -1.05) { $\mathsf{x}_{2}$ };
  
    \foreach [count=\n] \x/\y in {-2.177/-1.092, -2.310/-0.325, -2.207/0.368, -1.858/0.955, -1.178/1.463, 
                                  -0.891/1.780, -0.295/1.687, 0.244/1.431, 0.651/1.105, 1.066/1.167, 
                                  1.393/0.721, 0.971/0.228, 1.260/-0.140, 1.056/-0.296, 0.503/-0.760, 
                                  0.394/-0.874, 0.095/-0.908, -0.132/-0.901, -0.266/-0.434, -0.447/-0.503, 
                                  -0.553/-0.235, -0.304/-0.147, -0.488/0.065, -0.402/0.322, -0.201/0.373} {
      \ifnum \n>\m
        \colorlet{pointcolor}{lightteal}; 
      \else
        \colorlet{pointcolor}{midteal};
      \fi
      
      \ifnum \n>1
        \draw[color=midteal, -{Stealth[length=3, width=4]}, 
              line width=1, shorten >=1.25] (A) -- (\x, \y);
        \fill[color=pointcolor] (A) circle (0.05);
      \fi
    
      \ifnum \n=25
        \fill[color=pointcolor] (\x, \y) circle (0.05);
      \fi
      \coordinate (A) at (\x, \y);
    }
    
    \fill[lightteal] (0, 0) circle (0.05) node[right] { $x$ };
  \end{scope}

  \pgfmathsetmacro{\m}{22}
  \pgfmathsetmacro{\eps}{0.55}
    
  \begin{scope}[shift={(14, 0)}]
    \draw[white] (-3.5, -3) rectangle (3.5, 2.75);
  
    \draw[rounded corners=3, color=black, line width=1] (-3, -2) rectangle (3, 2);
    \node at (0, -2.5) { $X$ };
      
    \pgfmathsetseed{10}
    \draw[gray80, dashed, line width=1] 
      plot [smooth cycle, tension=0.75] coordinates { \randpoints{0}{-0.15}{1.95} } -- cycle;
    \draw[gray80, dashed, line width=1] 
      plot [smooth cycle, tension=0.75] coordinates { \randpoints{0.05}{0}{1.15} } -- cycle;
          
    \filldraw[darkteal, dashed, line width=1] 
      plot [smooth cycle, tension=0.75] coordinates { \randpoints{-0.05}{0.2}{\eps} } -- cycle;
    \filldraw[darkteal, dashed, line width=1] 
      plot [smooth cycle, tension=0.75] coordinates { \randpoints{-0.05}{0.8}{0.1} } -- cycle;
    \node[darkteal] at (-0.65, 0.75) { $\mathsf{x}_{3}$ };
  
    \foreach [count=\n] \x/\y in {-2.177/-1.092, -2.310/-0.325, -2.207/0.368, -1.858/0.955, -1.178/1.463, 
                                  -0.891/1.780, -0.295/1.687, 0.244/1.431, 0.651/1.105, 1.066/1.167, 
                                  1.393/0.721, 0.971/0.228, 1.260/-0.140, 1.056/-0.296, 0.503/-0.760, 
                                  0.394/-0.874, 0.095/-0.908, -0.132/-0.901, -0.266/-0.434, -0.447/-0.503, 
                                  -0.553/-0.235, -0.304/-0.147, -0.488/0.065, -0.402/0.322, -0.201/0.373} {
      \ifnum \n>\m
        \colorlet{pointcolor}{lightteal}; 
      \else
        \colorlet{pointcolor}{midteal};
      \fi
      
      \ifnum \n>1
        \draw[color=midteal, -{Stealth[length=3, width=4]}, 
              line width=1, shorten >=1.25] (A) -- (\x, \y);
        \fill[color=pointcolor] (A) circle (0.05);
      \fi
    
      \ifnum \n=25
        \fill[color=pointcolor] (\x, \y) circle (0.05);
      \fi
      \coordinate (A) at (\x, \y);
    }
    
    \fill[lightteal] (0, 0) circle (0.05) node[right] { $x$ };
  \end{scope}
    
\end{tikzpicture}

\end{document}  