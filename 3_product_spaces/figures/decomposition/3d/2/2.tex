\documentclass{standalone}
\usepackage{graphicx}	
\usepackage{amssymb, amsmath, amsthm}
\usepackage{color}

\usepackage{tikz}
\usetikzlibrary{math, fadings}

\definecolor{light}{RGB}{220, 188, 188}
\definecolor{mid}{RGB}{185, 124, 124}
\definecolor{dark}{RGB}{143, 39, 39}
\definecolor{darker}{RGB}{124, 0, 0}
\definecolor{highlight}{RGB}{180, 31, 180}
\definecolor{lightteal}{RGB}{186, 219, 219}
\definecolor{midteal}{RGB}{124, 186, 186}
\definecolor{darkteal}{RGB}{38, 142, 142}
\definecolor{darkerteal}{RGB}{0, 124, 124}
\definecolor{gray10}{gray}{0.1}
\definecolor{gray20}{gray}{0.2}
\definecolor{gray30}{gray}{0.3}
\definecolor{gray40}{gray}{0.4}
\definecolor{gray60}{gray}{0.6}
\definecolor{gray70}{gray}{0.7}
\definecolor{gray80}{gray}{0.8}
\definecolor{gray90}{gray}{0.9}
\definecolor{gray95}{gray}{0.95}

\tikzmath{
  function x(\x, \y, \z, \theta, \phi) {
    \c = cos(\theta);
    \s = sin(\theta);
    \xa = +\c * \x + \s * \z;
    \ya = \y;
    \za = -\s * \x + \c * \z;
    %
    \xb = \xa;
    %\yb = +cos(\phi) * \ya + sin(\phi) * \za;
    %\zb = -sin(\phi) * \ya + cos(\phi) * \za;
    %  
    return \xb;
  };
  function y(\x, \y, \z, \theta, \phi) {
    \c = cos(\theta);
    \s = sin(\theta);
    \xa = +\c * \x + \s * \z;
    \ya = \y;
    \za = -\s * \x + \c * \z;
    %
    %\xb = \xa;
    \yb = +cos(\phi) * \ya + sin(\phi) * \za;
    %\zb = -sin(\phi) * \ya + cos(\phi) * \za;
    %   
    return \yb;
  };
}

\begin{document}

\begin{tikzpicture}[scale=0.7]
  
  \pgfmathsetmacro{\theta}{50}
  \pgfmathsetmacro{\phi}{15}
   
  \pgfmathsetmacro{\N}{20}  
  \pgfmathsetmacro{\delta}{1 / \N}
  \pgfmathsetmacro{\x}{1 + \delta}
  \pgfmathsetmacro{\y}{1 + \delta}
  \pgfmathsetmacro{\z}{1 + \delta}
  
  \draw[white] (-8, -15) rectangle (16, 3);
  
  \node[anchor=west] at (9.5, 0 + 0.5) 
      { $ \{ \; \{ x_{1} \} \times \{ x_{2} \} \times X_{3} $ };
  \node[anchor=west] at (9.5, 0 - 0.5) 
      { $ \;\; \text{ for } (x_{1}, x_{2}) \in X_{1} \times X_{2} \; \}$ };
      
  \node at (-5.5, 0)
      { $X_{1} \times X_{2} \times X_{3}$ };
  %\node at (-5, 0 - 0.5)
  %    { $\displaystyle \bigcup_{x_{3} \in X_{3}} X_{1} \times X_{2} \times \{ x_{3} \}$ };


  \node[anchor=west] at (9.5, -6 + 0.5) 
      { $ \{ \; \{ x_{1} \} \times X_{2} \times \{ x_{3} \} $ };
  \node[anchor=west] at (9.5, -6 - 0.5) 
      { $ \;\; \text{ for } (x_{1}, x_{3}) \in X_{1} \times X_{3} \; \}$ };
  
  \node at (-5.5, -6)
      { $X_{1} \times X_{2} \times X_{3}$ };
  %\node at (-6, -6 - 0.5)
  %    { $\displaystyle \bigcup_{x_{2} \in X_{2}} X_{1} \times \{ x_{2} \} \times X_{3}$ };
   
   
  \node[anchor=west] at (9.5, -12 + 0.5) 
      { $ \{ \; X_{1} \times \{ x_{2} \} \times \{ x_{3} \} $ };
  \node[anchor=west] at (9.5, -12 - 0.5) 
      { $ \;\; \text{ for } (x_{2}, x_{3}) \in X_{2} \times X_{3} \; \}$ };   

  \node at (-5.5, -12)
      { $X_{1} \times X_{2} \times X_{3}$ };
  %\node at (-7, -12 - 0.5)
  %    { $\displaystyle \bigcup_{x_{1} \in X_{1}} \{ x_{1} \} \times X_{2} \times X_{3}$ };

  % X1 x X2
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-3, -3) rectangle (3, 3);
    
    \foreach \n in {0, 1, ..., \N} {
      \foreach \m in {0, 1, ..., \N} {
        \pgfmathsetmacro{\xi}{1 * (\n * 2 * \delta - 1)}
        \pgfmathsetmacro{\zeta}{-1 * (\m * 2 * \delta - 1)}
        
        \pgfmathsetmacro{\prop}{100 * (\n + \m) * \delta / 2}
        
        \colorlet{custom}{darkerteal!\prop!lightteal}; 
        \fill[custom] 
             ({x(\xi + \delta, -\y, \zeta + \delta, \theta, \phi)}, 
              {y(\xi + \delta, -\y, \zeta + \delta, \theta, \phi)})
          -- ({x(\xi + \delta, +\y, \zeta + \delta, \theta, \phi)}, 
              {y(\xi + \delta, +\y, \zeta + \delta, \theta, \phi)})
          -- ({x(\xi - \delta, +\y, \zeta + \delta, \theta, \phi)}, 
              {y(\xi - \delta, +\y, \zeta + \delta, \theta, \phi)})
          -- ({x(\xi - \delta, +\y, \zeta - \delta, \theta, \phi)}, 
              {y(\xi - \delta, +\y, \zeta - \delta, \theta, \phi)})
          -- ({x(\xi - \delta, -\y, \zeta - \delta, \theta, \phi)}, 
              {y(\xi - \delta, -\y, \zeta - \delta, \theta, \phi)})
          -- ({x(\xi + \delta, -\y, \zeta - \delta, \theta, \phi)}, 
              {y(\xi + \delta, -\y, \zeta - \delta, \theta, \phi)})
          --cycle;
      }              
    }
    
    \colorlet{custom}{darkerteal!140!lightteal};
    \draw[custom, line width=0.25]
         ({x(+\x, -\y, -\z, \theta, \phi)}, 
          {y(+\x, -\y, -\z, \theta, \phi)})
      -- ({x(+\x, -\y, +\z, \theta, \phi)}, 
          {y(+\x, -\y, +\z, \theta, \phi)})
      -- ({x(+\x, +\y, +\z, \theta, \phi)}, 
          {y(+\x, +\y, +\z, \theta, \phi)})
      -- ({x(-\x, +\y, +\z, \theta, \phi)}, 
          {y(-\x, +\y, +\z, \theta, \phi)})
      -- ({x(-\x, +\y, -\z, \theta, \phi)}, 
          {y(-\x, +\y, -\z, \theta, \phi)})
      -- ({x(-\x, -\y, -\z, \theta, \phi)}, 
          {y(-\x, -\y, -\z, \theta, \phi)})
      -- cycle;

    \draw[custom, line width=0.25]
         ({x(+\x, -\y, -\z, \theta, \phi)}, 
          {y(+\x, -\y, -\z, \theta, \phi)})
      -- ({x(+\x, +\y, -\z, \theta, \phi)}, 
          {y(+\x, +\y, -\z, \theta, \phi)})
      -- ({x(-\x, +\y, -\z, \theta, \phi)}, 
          {y(-\x, +\y, -\z, \theta, \phi)});

    \draw[custom, line width=0.25]
         ({x(+\x, +\y, -\z, \theta, \phi)}, 
          {y(+\x, +\y, -\z, \theta, \phi)})
      -- ({x(+\x, +\y, +\z, \theta, \phi)}, 
          {y(+\x, +\y, +\z, \theta, \phi)});
  \end{scope}
  

  \begin{scope}[shift={(6, 0)}]
    \draw[white] (-3, -3) rectangle (3, 3);
    
    \foreach \n in {0, 1, ..., \N} {
      \foreach \m in {0, 1, ..., \N} {
        \pgfmathsetmacro{\xi}{2 * (\n * 2 *  \delta - 1)}
        \pgfmathsetmacro{\zeta}{-2 * (\m * 2 * \delta - 1)}
        
        \pgfmathsetmacro{\prop}{100 * (\n + \m) * \delta / 2}
        
        \colorlet{custom}{darkerteal!\prop!lightteal}; 
        \fill[custom] 
             ({x(\xi + \delta, -\y, \zeta + \delta, \theta, \phi)}, 
              {y(\xi + \delta, -\y, \zeta + \delta, \theta, \phi)})
          -- ({x(\xi + \delta, +\y, \zeta + \delta, \theta, \phi)}, 
              {y(\xi + \delta, +\y, \zeta + \delta, \theta, \phi)})
          -- ({x(\xi - \delta, +\y, \zeta + \delta, \theta, \phi)}, 
              {y(\xi - \delta, +\y, \zeta + \delta, \theta, \phi)})
          -- ({x(\xi - \delta, +\y, \zeta - \delta, \theta, \phi)}, 
              {y(\xi - \delta, +\y, \zeta - \delta, \theta, \phi)})
          -- ({x(\xi - \delta, -\y, \zeta - \delta, \theta, \phi)}, 
              {y(\xi - \delta, -\y, \zeta - \delta, \theta, \phi)})
          -- ({x(\xi + \delta, -\y, \zeta - \delta, \theta, \phi)}, 
              {y(\xi + \delta, -\y, \zeta - \delta, \theta, \phi)})
          --cycle;
          
        \colorlet{custom}{darkerteal!140!lightteal};
        \draw[custom, line width=0.25]
               ({x(\xi + \delta, -\y, \zeta + \delta, \theta, \phi)}, 
                {y(\xi + \delta, -\y, \zeta + \delta, \theta, \phi)})
            -- ({x(\xi + \delta, +\y, \zeta + \delta, \theta, \phi)}, 
                {y(\xi + \delta, +\y, \zeta + \delta, \theta, \phi)})
            -- ({x(\xi - \delta, +\y, \zeta + \delta, \theta, \phi)}, 
                {y(\xi - \delta, +\y, \zeta + \delta, \theta, \phi)})
            -- ({x(\xi - \delta, +\y, \zeta - \delta, \theta, \phi)}, 
                {y(\xi - \delta, +\y, \zeta - \delta, \theta, \phi)})
            -- ({x(\xi - \delta, -\y, \zeta - \delta, \theta, \phi)}, 
                {y(\xi - \delta, -\y, \zeta - \delta, \theta, \phi)})
            -- ({x(\xi + \delta, -\y, \zeta - \delta, \theta, \phi)}, 
                {y(\xi + \delta, -\y, \zeta - \delta, \theta, \phi)})
            --cycle;

        \colorlet{custom}{darkerteal!140!lightteal};
        \draw[custom, line width=0.25]
               ({x(\xi + \delta, -\y, \zeta - \delta, \theta, \phi)}, 
                {y(\xi + \delta, -\y, \zeta - \delta, \theta, \phi)})
            -- ({x(\xi + \delta, +\y, \zeta - \delta, \theta, \phi)}, 
                {y(\xi + \delta, +\y, \zeta - \delta, \theta, \phi)})
            -- ({x(\xi - \delta, +\y, \zeta - \delta, \theta, \phi)}, 
                {y(\xi - \delta, +\y, \zeta - \delta, \theta, \phi)});
                
        \colorlet{custom}{darkerteal!140!lightteal};
        \draw[custom, line width=0.25]
               ({x(\xi + \delta, \y, \zeta - \delta, \theta, \phi)}, 
                {y(\xi + \delta, \y, \zeta - \delta, \theta, \phi)})
            -- ({x(\xi + \delta, \y, \zeta + \delta, \theta, \phi)}, 
                {y(\xi + \delta, \y, \zeta + \delta, \theta, \phi)});
      }              
    }
  \end{scope}

  % X1 x X3
  \begin{scope}[shift={(0, -6)}]
    \draw[white] (-3, -3) rectangle (3, 3);
    
    \foreach \n in {0, 1, ..., \N} {
      \foreach \m in {0, 1, ..., \N} {
        \pgfmathsetmacro{\eta}{1 * (\n * 2 * \delta - 1)}
        \pgfmathsetmacro{\zeta}{-1 * (\m * 2 * \delta - 1)}
        
        \pgfmathsetmacro{\prop}{100 * (\n + \m) * \delta / 2}
        
        \colorlet{custom}{darkerteal!\prop!lightteal}; 
        \fill[custom] 
             ({x(+\x, \eta - \delta, \zeta - \delta, \theta, \phi)}, 
              {y(+\x, \eta - \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(-\x, \eta - \delta, \zeta - \delta, \theta, \phi)}, 
              {y(-\x, \eta - \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(-\x, \eta + \delta, \zeta - \delta, \theta, \phi)}, 
              {y(-\x, \eta + \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(-\x, \eta + \delta, \zeta + \delta, \theta, \phi)}, 
              {y(-\x, \eta + \delta, \zeta + \delta, \theta, \phi)})
          -- ({x(+\x, \eta + \delta, \zeta + \delta, \theta, \phi)}, 
              {y(+\x, \eta + \delta, \zeta + \delta, \theta, \phi)})
          -- ({x(+\x, \eta - \delta, \zeta + \delta, \theta, \phi)}, 
              {y(+\x, \eta - \delta, \zeta + \delta, \theta, \phi)})
          -- cycle;
      }              
    }
    
    \colorlet{custom}{darkerteal!140!lightteal};
    \draw[custom, line width=0.25]
         ({x(+\x, -\y, -\z, \theta, \phi)}, 
          {y(+\x, -\y, -\z, \theta, \phi)})
      -- ({x(+\x, -\y, +\z, \theta, \phi)}, 
          {y(+\x, -\y, +\z, \theta, \phi)})
      -- ({x(+\x, +\y, +\z, \theta, \phi)}, 
          {y(+\x, +\y, +\z, \theta, \phi)})
      -- ({x(-\x, +\y, +\z, \theta, \phi)}, 
          {y(-\x, +\y, +\z, \theta, \phi)})
      -- ({x(-\x, +\y, -\z, \theta, \phi)}, 
          {y(-\x, +\y, -\z, \theta, \phi)})
      -- ({x(-\x, -\y, -\z, \theta, \phi)}, 
          {y(-\x, -\y, -\z, \theta, \phi)})
      -- cycle;

    \draw[custom, line width=0.25]
         ({x(+\x, -\y, -\z, \theta, \phi)}, 
          {y(+\x, -\y, -\z, \theta, \phi)})
      -- ({x(+\x, +\y, -\z, \theta, \phi)}, 
          {y(+\x, +\y, -\z, \theta, \phi)})
      -- ({x(-\x, +\y, -\z, \theta, \phi)}, 
          {y(-\x, +\y, -\z, \theta, \phi)});

    \draw[custom, line width=0.25]
         ({x(+\x, +\y, -\z, \theta, \phi)}, 
          {y(+\x, +\y, -\z, \theta, \phi)})
      -- ({x(+\x, +\y, +\z, \theta, \phi)}, 
          {y(+\x, +\y, +\z, \theta, \phi)});
  \end{scope}
  

  \begin{scope}[shift={(6, -6)}]
    \draw[white] (-3, -3) rectangle (3, 3);
    
    \foreach \n in {0, 1, ..., \N} {
      \foreach \m in {0, 1, ..., \N} {
        \pgfmathsetmacro{\eta}{2 * (\n * 2 * \delta - 1)}
        \pgfmathsetmacro{\zeta}{-2 * (\m * 2 * \delta - 1)}
        
        \pgfmathsetmacro{\prop}{100 * (\n + \m) * \delta / 2}
        
        \colorlet{custom}{darkerteal!\prop!lightteal}; 
        \fill[custom] 
             ({x(+\x, \eta - \delta, \zeta - \delta, \theta, \phi)}, 
              {y(+\x, \eta - \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(-\x, \eta - \delta, \zeta - \delta, \theta, \phi)}, 
              {y(-\x, \eta - \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(-\x, \eta + \delta, \zeta - \delta, \theta, \phi)}, 
              {y(-\x, \eta + \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(-\x, \eta + \delta, \zeta + \delta, \theta, \phi)}, 
              {y(-\x, \eta + \delta, \zeta + \delta, \theta, \phi)})
          -- ({x(+\x, \eta + \delta, \zeta + \delta, \theta, \phi)}, 
              {y(+\x, \eta + \delta, \zeta + \delta, \theta, \phi)})
          -- ({x(+\x, \eta - \delta, \zeta + \delta, \theta, \phi)}, 
              {y(+\x, \eta - \delta, \zeta + \delta, \theta, \phi)})
          -- cycle;
          
        \colorlet{custom}{darkerteal!140!lightteal};
        \draw[custom, line width=0.25]
             ({x(+\x, \eta - \delta, \zeta - \delta, \theta, \phi)}, 
              {y(+\x, \eta - \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(-\x, \eta - \delta, \zeta - \delta, \theta, \phi)}, 
              {y(-\x, \eta - \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(-\x, \eta + \delta, \zeta - \delta, \theta, \phi)}, 
              {y(-\x, \eta + \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(-\x, \eta + \delta, \zeta + \delta, \theta, \phi)}, 
              {y(-\x, \eta + \delta, \zeta + \delta, \theta, \phi)})
          -- ({x(+\x, \eta + \delta, \zeta + \delta, \theta, \phi)}, 
              {y(+\x, \eta + \delta, \zeta + \delta, \theta, \phi)})
          -- ({x(+\x, \eta - \delta, \zeta + \delta, \theta, \phi)}, 
              {y(+\x, \eta - \delta, \zeta + \delta, \theta, \phi)})
          -- cycle;

        \draw[custom, line width=0.25]
             ({x(-\x, \eta + \delta, \zeta - \delta, \theta, \phi)}, 
              {y(-\x, \eta + \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(+\x, \eta + \delta, \zeta - \delta, \theta, \phi)}, 
              {y(+\x, \eta + \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(+\x, \eta + \delta, \zeta + \delta, \theta, \phi)}, 
              {y(+\x, \eta + \delta, \zeta + \delta, \theta, \phi)});

        \draw[custom, line width=0.25]
             ({x(+\x, \eta + \delta, \zeta - \delta, \theta, \phi)}, 
              {y(+\x, \eta + \delta, \zeta - \delta, \theta, \phi)})
          -- ({x(+\x, \eta - \delta, \zeta - \delta, \theta, \phi)}, 
              {y(+\x, \eta - \delta, \zeta - \delta, \theta, \phi)});
      }             
    }
  \end{scope}
  
  % X2 x X3      
  \begin{scope}[shift={(0, -12)}]
    \draw[white] (-3, -3) rectangle (3, 3);
    
    \foreach \n in {0, 1, ..., \N} {
      \foreach \m in {0, 1, ..., \N} {
        \pgfmathsetmacro{\xi}{1 * (\n * 2 * \delta - 1)}
        \pgfmathsetmacro{\eta}{1 * (\m * 2 * \delta - 1)}
        
        \pgfmathsetmacro{\prop}{100 * (\n + \m) * \delta / 2}
        
        \colorlet{custom}{darkerteal!\prop!lightteal}; 
        \fill[custom] 
             ({x(\xi + \delta, \eta - \delta, -\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta - \delta, -\z, \theta, \phi)})
          -- ({x(\xi + \delta, \eta - \delta, +\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta - \delta, +\z, \theta, \phi)})
          -- ({x(\xi + \delta, \eta + \delta, +\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta + \delta, +\z, \theta, \phi)})
          -- ({x(\xi - \delta, \eta + \delta, +\z, \theta, \phi)}, 
              {y(\xi - \delta, \eta + \delta, +\z, \theta, \phi)})
          -- ({x(\xi - \delta, \eta + \delta, -\z, \theta, \phi)}, 
              {y(\xi - \delta, \eta + \delta, -\z, \theta, \phi)})
          -- ({x(\xi - \delta, \eta - \delta, -\z, \theta, \phi)}, 
              {y(\xi - \delta, \eta - \delta, -\z, \theta, \phi)})
          -- cycle;
      }               
    }
    
    \colorlet{custom}{darkerteal!140!lightteal};
    \draw[custom, line width=0.25]
         ({x(+\x, -\y, -\z, \theta, \phi)}, 
          {y(+\x, -\y, -\z, \theta, \phi)})
      -- ({x(+\x, -\y, +\z, \theta, \phi)}, 
          {y(+\x, -\y, +\z, \theta, \phi)})
      -- ({x(+\x, +\y, +\z, \theta, \phi)}, 
          {y(+\x, +\y, +\z, \theta, \phi)})
      -- ({x(-\x, +\y, +\z, \theta, \phi)}, 
          {y(-\x, +\y, +\z, \theta, \phi)})
      -- ({x(-\x, +\y, -\z, \theta, \phi)}, 
          {y(-\x, +\y, -\z, \theta, \phi)})
      -- ({x(-\x, -\y, -\z, \theta, \phi)}, 
          {y(-\x, -\y, -\z, \theta, \phi)})
      -- cycle;

    \draw[custom, line width=0.25]
         ({x(+\x, -\y, -\z, \theta, \phi)}, 
          {y(+\x, -\y, -\z, \theta, \phi)})
      -- ({x(+\x, +\y, -\z, \theta, \phi)}, 
          {y(+\x, +\y, -\z, \theta, \phi)})
      -- ({x(-\x, +\y, -\z, \theta, \phi)}, 
          {y(-\x, +\y, -\z, \theta, \phi)});

    \draw[custom, line width=0.25]
         ({x(+\x, +\y, -\z, \theta, \phi)}, 
          {y(+\x, +\y, -\z, \theta, \phi)})
      -- ({x(+\x, +\y, +\z, \theta, \phi)}, 
          {y(+\x, +\y, +\z, \theta, \phi)});
  \end{scope}
    
  \begin{scope}[shift={(6, -12)}]
    \draw[white] (-3, -3) rectangle (3, 3);
    
    \foreach \n in {0, 1, ..., \N} {
      \foreach \m in {0, 1, ..., \N} {
        \pgfmathsetmacro{\xi}{2 * (\n * 2 * \delta - 1)}
        \pgfmathsetmacro{\eta}{2 * (\m * 2 * \delta - 1)}
        
        \pgfmathsetmacro{\prop}{100 * (\n + \m) * \delta / 2}
        
        \colorlet{custom}{darkerteal!\prop!lightteal}; 
        \fill[custom] 
             ({x(\xi + \delta, \eta - \delta, -\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta - \delta, -\z, \theta, \phi)})
          -- ({x(\xi + \delta, \eta - \delta, +\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta - \delta, +\z, \theta, \phi)})
          -- ({x(\xi + \delta, \eta + \delta, +\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta + \delta, +\z, \theta, \phi)})
          -- ({x(\xi - \delta, \eta + \delta, +\z, \theta, \phi)}, 
              {y(\xi - \delta, \eta + \delta, +\z, \theta, \phi)})
          -- ({x(\xi - \delta, \eta + \delta, -\z, \theta, \phi)}, 
              {y(\xi - \delta, \eta + \delta, -\z, \theta, \phi)})
          -- ({x(\xi - \delta, \eta - \delta, -\z, \theta, \phi)}, 
              {y(\xi - \delta, \eta - \delta, -\z, \theta, \phi)})
          -- cycle;
          
          
        \colorlet{custom}{darkerteal!140!lightteal};
        \draw[custom, line width=0.25]
             ({x(\xi + \delta, \eta - \delta, -\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta - \delta, -\z, \theta, \phi)})
          -- ({x(\xi + \delta, \eta - \delta, +\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta - \delta, +\z, \theta, \phi)})
          -- ({x(\xi + \delta, \eta + \delta, +\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta + \delta, +\z, \theta, \phi)})
          -- ({x(\xi - \delta, \eta + \delta, +\z, \theta, \phi)}, 
              {y(\xi - \delta, \eta + \delta, +\z, \theta, \phi)})
          -- ({x(\xi - \delta, \eta + \delta, -\z, \theta, \phi)}, 
              {y(\xi - \delta, \eta + \delta, -\z, \theta, \phi)})
          -- ({x(\xi - \delta, \eta - \delta, -\z, \theta, \phi)}, 
              {y(\xi - \delta, \eta - \delta, -\z, \theta, \phi)})
          -- cycle;
  
        \draw[custom, line width=0.25] 
             ({x(\xi + \delta, \eta - \delta, -\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta - \delta, -\z, \theta, \phi)})
          -- ({x(\xi + \delta, \eta + \delta, -\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta + \delta, -\z, \theta, \phi)})           
          -- ({x(\xi - \delta, \eta + \delta, -\z, \theta, \phi)}, 
              {y(\xi - \delta, \eta + \delta, -\z, \theta, \phi)});
  
        \draw[custom, line width=0.25]       
             ({x(\xi + \delta, \eta + \delta, -\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta + \delta, -\z, \theta, \phi)}) 
          -- ({x(\xi + \delta, \eta + \delta, +\z, \theta, \phi)}, 
              {y(\xi + \delta, \eta + \delta, +\z, \theta, \phi)});  
      }               
    }
  \end{scope}

\end{tikzpicture}

\end{document}  