...
\begin{tikzpicture}[scale=1]
  ...
  \begin{scope}[shift={(0, 0)}]
    ...
    \node[gray80, anchor=west] at (3, \y) { $\pi_{1} = \displaystyle \max_{x \in \mathsf{I}_{1}} \pi(x)$ };

    \pgfmathsetmacro{\xc}{-0.1};
    \draw[gray80, dashed, line width=1] (\xc, -2) -- (\xc, 2);
    
    \pgfmathsetmacro{\xd}{0.1};
    \draw[gray80, dashed, line width=1] (\xd, -2) -- (\xd, 2);
    
    \pgfmathsetmacro{\y}{7 * density(\xc) - 2};
    \draw[gray80, dashed, line width=1] (-3, \y) -- (3, \y);
    \node[gray80, anchor=west] at (3, \y) { $\pi_{2} = \displaystyle \min_{x \in \mathsf{I}_{2}} \pi(x)$ };
    ...
    \node[white] at ({0.5 * (\xa + \xb) + 0.75}, -1.5) { $\pi( \mathsf{I}_{1} )$ };
    \node[white] at ({0.5 * (\xc + \xd)}, -0.25) { $\contour{white}{$\pi( \mathsf{I}_{2}$} )$ };
    \node[dark] at ({0.5 * (\xc + \xd)}, -0.25) { $\pi( \mathsf{I}_{2} )$ };  
