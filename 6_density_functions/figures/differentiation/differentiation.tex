\documentclass{standalone}
\usepackage{graphicx}	
\usepackage{amssymb, amsmath}
\usepackage{color}

\usepackage{tikz}
\usetikzlibrary{intersections, backgrounds, math}
\usepackage{pgfmath}

\definecolor{light}{RGB}{220, 188, 188}
\definecolor{mid}{RGB}{185, 124, 124}
\definecolor{dark}{RGB}{143, 39, 39}
\definecolor{highlight}{RGB}{180, 31, 180}
\definecolor{lightteal}{RGB}{186, 219, 219}
\definecolor{midteal}{RGB}{124, 186, 186}
\definecolor{darkteal}{RGB}{38, 142, 142}
\definecolor{darkerteal}{RGB}{0, 124, 124}
\definecolor{gray10}{gray}{0.1}
\definecolor{gray20}{gray}{0.2}
\definecolor{gray30}{gray}{0.3}
\definecolor{gray40}{gray}{0.4}
\definecolor{gray60}{gray}{0.6}
\definecolor{gray70}{gray}{0.7}
\definecolor{gray80}{gray}{0.8}
\definecolor{gray90}{gray}{0.9}
\definecolor{gray95}{gray}{0.95}

\tikzmath{
  function f(\x) {
    return (\x * \x * \x - 2 * \x) / (1 + 0.25 * \x * \x);
  };
  function dfdx(\x) {
    return   (0.25 * \x * \x * \x * \x + 3.5 * \x * \x - 2) 
           / ((1 + 0.25 * \x * \x) * (1 + 0.25 * \x * \x));
  };
  function df2dx2(\x) {
    return 2 * \x * (-0.25 * \x * \x + 3) * (1 + 0.5)
           / ((1 + 0.25 * \x * \x) * (1 + 0.25 * \x * \x) * (1 + 0.25 * \x * \x));
  };
}

\begin{document}

\begin{tikzpicture}[scale=1.0]

   \pgfmathsetmacro{\ys}{0.31};

  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-4.5, -3.5) rectangle (4.25, 3);

    \pgfmathsetmacro{\x}{-2}
    \draw[gray70, dashed, line width=1.5] (\x, -2) -- (\x, {\ys * f(\x)});
    \node[gray70] at (\x, -2.5) { $x_{1}$ };
    
    \pgfmathsetmacro{\x}{0.25}
    \draw[gray70, dashed, line width=1.5] (\x, -2) -- (\x, {\ys * f(\x)});
    \node[gray70] at (\x, -2.5) { $x_{2}$ };

    \draw[domain={-3:3}, smooth, samples=150, line width=1.25, variable=\x, color=darkteal] 
      plot ({\x},{\ys * f(\x)});

    \draw [->, >=stealth, line width=1.25] (-3.00, -2.015) -- +(0, 4);
    \draw [->, >=stealth, line width=1.25] (-3.015, -2.00) -- +(6, 0);
    
    \node at (-3.75, 0) { $f(x)$ };
    \node at (0, -3) { $x$ };
  \end{scope}

  \begin{scope}[shift={(8.75, 0)}]
    \draw[white] (-4.5, -3.5) rectangle (4.25, 3);
   
    \draw[gray70, dashed, line width=1.5] (-3, 0) -- (3, 0);
    \node[gray70] at (3.25, 0) { $0$ };

    \pgfmathsetmacro{\x}{-2}
    \draw[gray70, dashed, line width=1.5] (\x, -2) -- (\x, {\ys * dfdx(\x)});
    \node[gray70] at (\x, -2.5) { $x_{1}$ };
    
    \pgfmathsetmacro{\x}{0.25}
    \draw[gray70, dashed, line width=1.5] (\x, -2) -- (\x, {\ys * dfdx(\x)});
    \node[gray70] at (\x, -2.5) { $x_{2}$ };

    \begin{scope}[shift={(0, 0)}]
      \clip (-3, -2) rectangle (3, 2);
      \draw[domain={-3:3}, smooth, samples=150, line width=1, variable=\x, color=darkteal] 
        plot ({\x},{\ys * ( dfdx(\x)) )});
    \end{scope}
    
    \draw [->, >=stealth, line width=1.25] (-3.00, -2.015) -- +(0, 4);
    \draw [->, >=stealth, line width=1.25] (-3.015, -2.00) -- +(6, 0);
    
    \node at (-3.75, 0) { $\frac{ \mathrm{d} f}{ \mathrm{d} x} (x)$ };
    \node at (0, -3) { $x$ };
  \end{scope}

  
\end{tikzpicture}

\end{document}  