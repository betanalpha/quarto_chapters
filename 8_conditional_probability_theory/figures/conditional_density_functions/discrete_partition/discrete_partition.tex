...
\begin{tikzpicture}[scale=1]
  ...
  \begin{scope}[shift={(0, 0)}]
    ...
    \begin{axis}[view={\theta}{\phi},
                 xmin=-2.5, xmax=+2.5, xtick=\empty, 
                 ymin=-2.5, ymax=+2.5, ytick=\empty,
                 zmin=0, zmax=0.105, ztick=\empty,
                 axis lines=none,              
                 line width=1,
                 colormap name=reds] 
      %\addplot3[surf, shader=interp, samples=70, fill opacity=0.8, domain=-2.5:2.5] 
      %  {multinormal(x, y) * I(x, y, 1)};
    \end{axis}
    ...
    \foreach \d in {-2} {
      \begin{axis}[view={\theta}{\phi},
        ...
        \addplot3[surf, shader=interp, samples=50, fill opacity=0.8, domain=-2.5:2.5] 
          ( {\x}, {Y(\x, \y, \d, \delta)}, {multinormal(\x, Y(\x, \y, \d, \delta))} );
        \addplot3 [domain=-2.5:2.5, samples=60, samples y=0, dark]
          ( {\x}, {ymax(\x, \d, \delta)}, {multinormal(\x, ymax(\x, \d, \delta))} );
      \end{axis}
    }

    \foreach \d in {-1, 0, 1, 2, 3} {
      \begin{axis}[view={\theta}{\phi},
                   xmin=-2.5, xmax=+2.5, xtick=\empty, 
                   ymin=-2.5, ymax=+2.5, ytick=\empty,
                   zmin=0,    zmax=0.105, ztick=\empty,
                   ...
                   colormap name=reds]
        \addplot3 [domain=-2.5:2.5, samples=60, samples y=0, dark]
          ( {\x}, {ymin(\x, \d, \delta)}, {multinormal(\x, ymin(\x, \d, \delta))} );
        \addplot3[surf, shader=interp, samples=50, fill opacity=0.8, domain=-2.5:2.5] 
          ( {\x}, {Y(\x, \y, \d, \delta)}, {multinormal(\x, Y(\x, \y, \d, \delta))} );
        \addplot3 [domain=-2.5:2.5, samples=60, samples y=0, dark]
          ( {\x}, {ymax(\x, \d, \delta)}, {multinormal(\x, ymax(\x, \d, \delta))} );
