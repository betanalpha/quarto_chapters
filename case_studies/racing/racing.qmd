...
```{r}
skill_comp <- function(active_skill, ...) {
  other_skills <- c(...)
...
eval_forfeit_win <- function(status1, status2) {
  as.numeric(status1 == 0 & status2 == 1)
}
forfeit_win_samples <-
  util$eval_expectand_pushforward(samples,
                                  eval_forfeit_win,
                                  list('status1' = status_name1,
                                       'status2' = status_name2))
p_forfeit_win <- util$ensemble_mcmc_est(forfeit_win_samples)[1]

eval_no_forfeits <- function(status1, status2) {
  as.numeric(status1 == 0 & status2 == 0)
}
no_forfeits_samples <-
  util$eval_expectand_pushforward(samples,
                                  eval_no_forfeits,
