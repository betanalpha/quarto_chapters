...
\begin{tikzpicture}[scale=0.2, thick]
  ...
  \draw[white] (-22, -14) rectangle (28, 22);
  ...
  \coordinate (H) at (-16, -8);
  \coordinate (I) at (-16, 0);
  \coordinate (J) at (-16, 8);
  
  \coordinate (K) at (22, 16);
  ...
  \draw[dark, line width=1.25, rounded corners=4] (-20, -12) rectangle (-12, 12);

  \foreach \B/\E in {A/B, B/C, D/B, E/C, F/C, F/G, D/I, H/I, I/J} {
    \draw[-{Stealth[length=6pt, width=6pt]}, shorten <=15, shorten >=15, color=mid, line width=1.5] (\B) -- (\E);
  }

  \coordinate (delta) at (\r, \r);
  \filldraw[fill=dark, draw=dark, line width=1.5, anchor=center] ($(A) - (delta)$) rectangle ($(A) + (delta)$)
  node[color=white, midway] { $x_{n}$ };

  \filldraw[fill=white, draw=dark, dashed, line width=1.5] (B) circle (\r)
  node[color=black] { $\theta_{n}$ };
  
  \filldraw[fill=dark, draw=dark, line width=1.5] (C) circle (\r)
  ...
  node[color=white] { $y^{\text{aux}}_{n}$ };
  
  \filldraw[fill=white, draw=dark, line width=1.5, anchor=center] ($(H) - (delta)$) rectangle ($(H) + (delta)$)
  node[color=black, midway] { $x^{\text{hyp}}_{n}$ };
  
  \filldraw[fill=white, draw=dark, dashed, line width=1.5] (I) circle (\r)
  node[color=black] { $\theta_{n}$ };
  
  \filldraw[fill=white, draw=dark, line width=1.5] (J) circle (\r)
  node[color=black] { $y^{\text{hyp}}_{n}$ };
