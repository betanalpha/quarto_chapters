...
Just as elements of a product sets are uniquely specified with an element from
each of the component sets, subsets of product sets are uniquely specified with
a subset from each of the component sets.  These product subsets
...
Set operations on product subsets are defined by applying them to each component
subset individually and then combining the results.  For example given the
product subsets
$$
...
These product set operations also introduce a certain geometric intuition to
product subsets.  For example considering decomposing the product set
...
$$
The intersection of these two product subsets is then just the product of
...
One way to interpret product sets is geometrically.  From this perspective
(a, b) we first lift each component set to the product space before (c)
constructing the product set from their overlap.
:::
...
Commutativity of the product metric follows the commutativity of the component
metrics,
\begin{align*}
d(x, x')
...
Moreover because the component open subsets are closed under countable unions
and finite intersections these product subsets will be as well.  For example
given any collection of open component subsets
$$
...
$$
Consequently the union of any collection of open product subsets gives another
open product subset,
\begin{align*}
\cup_{j} \mathsf{x}_{j}
&=
...
$\times_{i = 1}^{I} \mathsf{x}_{i}$ with $\mathsf{x}_{i} \in \mathfrak{t}_{i}$
satisfies all of the properties of a topology.  We refer to this topology as a
**product topology**.
...
\end{alignat*}
That said we can work through the component spaces in _any_ order.  This gives
...
spaces.  We can say much more, howver, about functions that naturally harmonize
...
every function between product spaces
$f : \times_{i = 1}^{I} X_{i} \rightarrow \times_{j = 1}^{J} X_{J}$ decomposes
...
& y_{j} = f_{j}(x_{1}, \ldots, x_{i}, \ldots, x_{I}) &.
\end{alignat*}
Those component functions, however, can also be defined as the composition of
$f$ with each projection function on the output space,
$$
