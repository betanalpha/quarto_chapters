...
Just as elements of a product sets are uniquely specified with an
element from each of the component sets, subsets of product sets are
uniquely specified with a subset from each of the component sets. These
product subsets \(\mathsf{x} \subset \times_{i = 1}^{I} X_{i}\) are
often written as \[
\mathsf{x}
=
...
Set operations on product subsets are defined by applying them to each
component subset individually and then combining the results. For
example given the product subsets \[
\mathsf{x} = \times_{i = 1}^{I} \mathsf{x}_{i}
\] and \[
\mathsf{x}' = \times_{i = 1}^{I} \mathsf{x}'_{i}
\] we can define their union as the product subset \[
\mathsf{x} \cup \mathsf{x}'
=
\times_{i = 1}^{I} \mathsf{x}_{i} \cup \mathsf{x}'_{i}
\] and their intersection as the product subset \[
\mathsf{x} \cap \mathsf{x}'
=
\times_{i = 1}^{I} \mathsf{x}_{i} \cap \mathsf{x}'_{i}.
\]

These product set operations also introduce a certain geometric
intuition to product subsets. For example considering decomposing the
product set \(X_{1} \times X_{2}\) into replications of \(X_{1}\), \[
X_{1} \times X_{2} = \bigcup_{x_{2} \in X_{2}} X_{1} \times \{ x_{2} \}.
...
\] The intersection of these two product subsets is then just the
...
\end{align*} In other words we can interpret the product subset
\(\mathsf{x}_{1} \times \mathsf{x}_{2}\) as the overlap of the shadows
that \(\mathsf{x}_{1}\) and \(\mathsf{x}_{2}\) cast into the full
product set (Figure~\ref{fig-product-set}).
...
\caption{\label{fig-product-set}One way to interpret product sets is
geometrically. From this perspective (a, b) we first lift each component
set to the product space before (c) constructing the product set from
their overlap.}
...
Commutativity of the product metric follows the commutativity of the
...
Moreover because the component open subsets are closed under countable
unions and finite intersections these product subsets will be as well.
For example given any collection of open component subsets \[
\{ \mathsf{x}_{1, i}, \ldots, \mathsf{x}_{j, i}, \ldots \}
\] we have closure under unions, \[
\cup_{j} \mathsf{x}_{j, i} = \mathsf{x}_{\cup, i} \in \mathfrak{t}_{i}.
\] Consequently the union of any collection of open product subsets
gives another open product subset, \begin{align*}
\cup_{j} \mathsf{x}_{j}
&=
\cup_{j} \left( \times_{i = 1}^{I} \mathsf{x}_{j, i} \right)
\\
&=
\times_{i = 1}^{I} \left( \cup_{j} \mathsf{x}_{j, i} \right)
\\
&=
\times_{i = 1}^{I} \left( \mathsf{x}_{\cup, i} \right).
\end{align*}

In other words the collection of all open product subsets
\(\times_{i = 1}^{I} \mathsf{x}_{i}\) with
\(\mathsf{x}_{i} \in \mathfrak{t}_{i}\) satisfies all of the properties
of a topology. We refer to this topology as a \textbf{product topology}.
...
\emph{any} order. This gives flexibility allows us to construct joint
elements in whichever way is most convenient in a given application.
...
general input spaces. We can say much more, howver, about functions that
naturally harmonize with the component structure of an input product
space.
...
previous section every function between product spaces
\(f : \times_{i = 1}^{I} X_{i} \rightarrow \times_{j = 1}^{J} X_{J}\)
decomposes into component functions \begin{alignat*}{6}
f_{j} :\; & \times_{i = 1}^{I} X_{i} & &\rightarrow& \; & Y_{j} &
\\
& (x_{1}, \ldots, x_{i}, \ldots, x_{I}) & &\mapsto&
& y_{j} = f_{j}(x_{1}, \ldots, x_{i}, \ldots, x_{I}) &.
\end{alignat*} Those component functions, however, can also be defined
as the composition of \(f\) with each projection function on the output
space, \[
