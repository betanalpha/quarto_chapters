...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    ...
    \node (C) at (-2 * \d, 1.5 * \d) [unobscirc] { $\hphantom{y_{c, n}}\vphantom{y_{c, n}}$ };
    \node     at (-2 * \d, 1.5 * \d) { $y_{n}$ };
    \node (D) at (-1.35 * \d, 3 * \d) [obsell] { $\hphantom{z_{c, n} = 1}$ };
    \node[white] at (-1.35 * \d, 3 * \d) { $z_{n} = 1$ };

    \node (E) at (+1 * \d, 1.5 * \d) [unobscirc] { $y_{c, n}$ };
    \node (F) at (+1.65 * \d, 3 * \d) [obsell] { $z_{c, n} = 1$ };

    \foreach \B/\E in {A/C, B/D, C/D, B/F, E/F} {
      \draw[->, >=stealth, color=mid, line width=1.5] (\B) -- (\E);
    }
    
    \node at (0, -0.85 * \d) { $      \prod_{n = 1}^{N} p(y_{n}, \phi, \psi \mid z_{n} = 1)
                                \cdot \prod_{n = 1}^{N_{c}} p(y_{c, n}, \psi \mid z_{c, n} = 1)$ };
