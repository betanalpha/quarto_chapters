...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    ...
    \node (B) at (+\d, 0) [obscirc] { $\psi$ };
    
    \node (C) at (-\d, 1.5 * \d) [obscirc] { $y$ };

    \node (D) at (+\d, 3 * \d) [unobsell] { $z = 1$ };

    \foreach \B/\E in {A/C, B/D, C/D} {
      \draw[->, >=stealth, color=mid, line width=1.5] (\B) -- (\E);
    }
    
    \fill[white, opacity=0.75] (-1.5 * \d, -0.5 * \d) rectangle (-0.5 * \d, +1.16 * \d);
    
    \node at (0, -0.85 * \d) { $p(z = 1 \mid y, \phi)$ };
