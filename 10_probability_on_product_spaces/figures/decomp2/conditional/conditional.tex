...
\tikzmath{
  function joint(\x, \y, \sx, \sy, \r) {
    \Qn = (\sx * \x)^2 - 2 * \r * \sx * \sy * \x * \y + (\sy * \y)^2;
    \Qd = (\sx * \sy)^2 * (1 - \r^2);
    return 0.15915494 * exp(-0.5 * \Qn / \Qd) / sqrt(\Qd);
  };
  function conditional(\x, \y, \sx, \sy, \r) {
    \Qn = (\x - \r * \sx * \y / \sy)^2;
    \Qd = \sx^2 * (1 - \r^2);
    return 0.39894228 * exp(-0.5 * \Qn / \Qd) / sqrt(\Qd);
  };
}
...
\begin{tikzpicture}[scale=1]
  ...
  \begin{scope}[shift={(0, 0)}]
    ...
    \node[above] at (B) { $p( x_{1} \mid x_{2} )$ };
    ...
    \foreach \yy in {-5, -4, ..., 5} {
      \begin{axis}[view={\theta}{\phi},
                   xmin=-5, xmax=5, xtick=\empty, 
                   ymin=-5, ymax=5, ytick=\empty,
                   zmin=0, zmax=0.65, ztick=\empty,
                   axis lines=none]
        \addplot3 [domain=-5:5, samples=120, samples y=0, dark, line width=1]
          ( {x}, {\yy}, { conditional(x, \yy, \sx, \sy, \r) } );
      \end{axis}
    }
