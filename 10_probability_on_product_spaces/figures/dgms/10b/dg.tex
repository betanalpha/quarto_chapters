...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-2.75 * \d, -1 * \d) rectangle (7.25 * \d, 5.5 * \d);

    \node (A) at (0 * \d, +0 * \d) [unobscirc] { $x_{2}$ };
    
    \node (B) at (+1.75 * \d, +1.5 * \d) [unobscirc] { $x_{3}$ };

    \node (C) at (-1.75 * \d, +3 * \d) [obscirc] { $x_{1}$ };
    
    \node (D) at (+0 * \d, +4.5 * \d) [obscirc] { $x_{4}$ };        

    \foreach \B/\E in {A/B, B/C, B/D, C/D} {
      \draw[->, >=stealth, color=mid, line width=1.5] (\B) -- (\E);
    }
    

    \node[anchor=west] at (3.25 * \d, 3.1 * \d) { $p( x_{2}, x_{3} \mid \tilde{x}_{1}, \tilde{x}_{4} )$ };    
    \node[anchor=west] at (3.25 * \d, 2.7 * \d) { $\quad \propto \;\, p( \tilde{x}_{1}, x_{2}, x_{3}, \tilde{x}_{4} )$ };
    \node[anchor=west] at (3.25 * \d, 2.3 * \d) { $\quad \propto \;\, p( \tilde{x}_{4} \mid \tilde{x}_{1}, x_{2} )$ }; 
    \node[anchor=west] at (3.25 * \d, 1.9 * \d) { $\quad\quad \cdot\, p( \tilde{x}_{1} \mid x_{3} )$ };
    \node[anchor=west] at (3.25 * \d, 1.5 * \d) { $\quad\quad \cdot\,  p( x_{3} \mid x_{2} )$ };
    \node[anchor=west] at (3.25 * \d, 1.1 * \d) { $\quad\quad \cdot\,  p( x_{2} )$ };
