...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-2.75 * \d, -0.5 * \d) rectangle (7.5 * \d, 7 * \d);

    \node (A) at (0 * \d, +0.5 * \d) [unobscirc] { $x_{2}$ };
    
    \node (B) at (+1.75 * \d, +3 * \d) [unobscirc] { $x_{1}$ };

    \node (C) at (-1.75 * \d, +1.5 * \d) [unobscirc] { $x_{3}$ };
    
    \node (D) at (+0 * \d, +4.5 * \d) [pushunobscirc] { $y$ };  

    \node (E) at (+0 * \d, +6 * \d) [unobscirc] { $x_{4}$ };     

    \foreach \B/\E in {A/C, C/B, B/D, C/D, D/E} {
      \draw[->, >=stealth, color=mid, line width=1.5] (\B) -- (\E);
    }
    
        
    \node[anchor=west] at (3.25 * \d, 3.7 * \d) { $p( x_{1}, x_{2}, x_{3}, x_{4}; y )$ };
    \node[anchor=west] at (3.25 * \d, 3.3 * \d) { $\quad = \;\, p( x_{4} \mid y = f(x_{1}, x_{2} ) )$ }; 
    \node[anchor=west] at (3.25 * \d, 2.9 * \d) { $\quad\quad \cdot\, p( x_{1} \mid x_{3} )$ };
    \node[anchor=west] at (3.25 * \d, 2.5 * \d) { $\quad\quad \cdot\,  p( x_{3} \mid x_{2} )$ };
    \node[anchor=west] at (3.25 * \d, 2.1 * \d) { $\quad\quad \cdot\,  p( x_{2} )$ };
