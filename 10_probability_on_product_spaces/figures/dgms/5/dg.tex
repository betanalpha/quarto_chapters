...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-5 * \d, -1.5 * \d) rectangle (7 * \d, 5.5 * \d);
    ...
    %\draw[light, dashed, rounded corners=1, line width=1.5] 
    %  (-4.5 * \d, +1.25 * \d) rectangle (+6 * \d, +2.75 * \d);
    %\node[align=center] at (5 * \d, 2 * \d) { Intermediate\\Nodes };
         
    \draw[light, dashed, rounded corners=1, line width=1.5] 
      (-4.5 * \d, -0.75 * \d) rectangle (+6 * \d, +0.75 * \d);
    \node[align=center] at (5 * \d, 0 * \d) { Root\\Node };
    
    \node (A) at (0 * \d, 0 * \d) [unobscirc] {  };
    
    \node (B) at (-2.25 * \d, +2 * \d) [unobscirc] {  };

    \node (C) at (+2.25 * \d, +2 * \d) [unobscirc] {  };
    
    \node (D) at (-0.75 * \d, +4 * \d) [unobscirc] {  };
    \node (E) at (-2.25 * \d, +4 * \d) [unobscirc] {  };
    \node (F) at (-3.75 * \d, +4 * \d) [unobscirc] {  };
    
    \node (G) at (+2.25 * \d, +4 * \d) [unobscirc] {  };
    
    \foreach \B/\E in {A/B, A/C, B/C, B/D, B/E, B/F, C/G} {
