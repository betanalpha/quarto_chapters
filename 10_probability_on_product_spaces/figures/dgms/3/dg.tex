...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-2 * \d, -2 * \d) rectangle (2 * \d, 2 * \d);

    \node (A) at (-1 * \d, -1 * \d) [unobscirc] {  };
    
    \node (B) at (+1 * \d, -1* \d) [unobscirc] {  };

    \node (C) at (+1 * \d, +1 * \d) [unobscirc] {  };
    
    \node (D) at (-1 * \d, +1 * \d) [unobscirc] {  };
    
    
    \draw[->, >=stealth, light, line width=1.5] (0.15 * \d, -0.6 * \d) arc (230:-120:2);
 
    \foreach \B/\E in {A/C, B/A, C/B, D/A} {
