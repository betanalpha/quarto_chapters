...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-3 * \d, -1.25 * \d) rectangle (11.25 * \d, 4 * \d);

    \node (A) at (0 * \d, +0 * \d) [unobscirc] { $x_{2}$ };
    
    \node (B) at (+1.75 * \d, +1.5 * \d) [unobsell] { $x_{1}, x_{3}$ };

    \node (C) at (-1.75 * \d, +3 * \d) [unobscirc] { $x_{4}$ };
    
    \foreach \B/\E in {A/B, A/C, B/C} {
      \draw[->, >=stealth, color=mid, line width=1.5] (\B) -- (\E);
    }
    
    \node[anchor=west] at (3.5 * \d, 3.1 * \d) { $p( x_{1}, x_{2}, x_{3}, x_{4} )$ };
    \node[anchor=west] at (3.5 * \d, 2.7 * \d) { $\quad = p( x_{4} \mid x_{1}, x_{2} ) \, 
                                                          p( x_{1}, x_{3} \mid x_{2} ) \, p( x_{2} )$ };

    \node[anchor=west] at (3.5 * \d, 1.7 * \d) { $p( x_{1}, x_{2}, x_{3}, x_{4} )$ };
    \node[anchor=west] at (3.5 * \d, 1.3 * \d) { $\quad = p( x_{4} \mid x_{3}, x_{2} ) \, 
                                                          p( x_{1}, x_{3} \mid x_{2} ) \, p( x_{2} )$ };
                                                          
    \node[anchor=west] at (3.5 * \d,  0.3 * \d) { $p( x_{1}, x_{2}, x_{3}, x_{4} )$ };
    \node[anchor=west] at (3.5 * \d, -0.1 * \d) { $\quad = p( x_{4} \mid x_{1}, x_{2}, x_{3} ) \, 
                                                          p( x_{1}, x_{3} \mid x_{2} ) \, p( x_{2} )$ };
  \end{scope}
