...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-2.75 * \d, -1 * \d) rectangle (7 * \d, 5.5 * \d);
    ...
    \node (E) at (2.75 * \d, 1 * \d) [unobsrec] { $y$ };      

    \foreach \B/\E in {A/C, C/B, B/D, C/D, E/B} {
      \draw[->, >=stealth, color=mid, line width=1.5] (\B) -- (\E);
    }
    
        
    \node[anchor=west] at (4 * \d, 3.1 * \d) { $p( x_{1}, x_{2}, x_{3}, x_{4}; y )$ };
    \node[anchor=west] at (4 * \d, 2.7 * \d) { $\quad = \;\, p( x_{4} \mid x_{1}, x_{2} )$ }; 
    \node[anchor=west] at (4 * \d, 2.3 * \d) { $\quad\quad \cdot\, p( x_{1} \mid x_{3}; \, y )$ };
    \node[anchor=west] at (4 * \d, 1.9 * \d) { $\quad\quad \cdot\,  p( x_{3} \mid x_{2} )$ };
    \node[anchor=west] at (4 * \d, 1.5 * \d) { $\quad\quad \cdot\,  p( x_{2} )$ };
