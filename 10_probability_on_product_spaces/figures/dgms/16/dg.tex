...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    ...
    \draw[dark, rounded corners=3, line width=1.5] 
      (-3 * \d, 3.25 * \d) rectangle (+1.25 * \d, +4.75 * \d);
    \node[align=center] at (-1.75 * \d, 4 * \d) { $j = \{ 1, \ldots, J_{i} \}$ };
    
    \draw[dark, rounded corners=3, line width=1.5] 
      (-3.25 * \d, 1 * \d) rectangle (+1.5 * \d, +5 * \d);
    \node[align=center] at (-1.75 * \d, 2 * \d) { $i = \{ 1, \ldots, I \}$ };

    \node (A) at (0 * \d, 0 * \d) [unobscirc] { $x_{0}$ };
    
    \node (B) at (0 * \d, +2 * \d) [unobscirc] { $x_{i}$ };

    \node (D) at (0 * \d, +4 * \d) [unobscirc] { $x_{i, j}$ };
            
    \foreach \B/\E in {A/B, B/D} {
