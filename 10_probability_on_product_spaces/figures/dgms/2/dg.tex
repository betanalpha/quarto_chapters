...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    \draw[white] (-2 * \d, -2.75 * \d) rectangle (2 * \d, 0 * \d);

    \node (A) at (-1 * \d, -1 * \d) [unobscirc] { $1$ };
    \node[align=center] at (-1 * \d, -2 * \d) { Parent of\\Node 2 };
    
    \node (B) at (+1 * \d, -1 * \d) [unobscirc] { $2$ };
    \node[align=center] at (+1 * \d, -2 * \d) { Child of\\Node 1 };
