...
\begin{tikzpicture}[scale=1, thick]
  ...
  \pgfmathsetmacro{\dy}{0.25}
  \pgfmathsetmacro{\tilt}{0.2}
  \foreach \x/\y/\p [count=\n] in {0/0/0.15, -2/-0.75/0.05, 2/1/0.3, -1.5/0.75/0.3, 1.5/-1/0.2} {
    \begin{scope}[shift={(\x, \y)}]
      \drawcube{\dx}{\dy}{4 * \p}{\tilt}
      \node[white] at (0, {4 * \p}) { $m_{\n}$ };
    \end{scope}
  }
  
  \node[dark] at (-4, 0.5 * 0.75) { $\mu ( \mathsf{x}_{1} ) = m_1 + m_4$ };
  \node[dark] at (4, -0.5) { $\mu ( \mathsf{x}_{2} ) = m_1 + m_5$ };
  
  \node[dark] at (0, 3) { $\mu ( \mathsf{x}_{1} \cup \mathsf{x}_{2} ) = m_1 + m_4 + m_5$ };
  \node[dark] at (0, -2) { $\mu ( \mathsf{x}_{1} \cap \mathsf{x}_{2} ) = m_1$ };
