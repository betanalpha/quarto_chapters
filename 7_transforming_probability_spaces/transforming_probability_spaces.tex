...
modified when we transform the ambient set in In
\href{http://www.symplectomorphic.com/assets/chapters_html/spaces.html}{Chapter
2}. In this chapter, we'll learn how measure-theoretic structures are
transformed, including probability distributions, expectation values,
...
\(f : X \rightarrow Y\), we can always push subsets in \(X\) forward to
...
\end{alignat*} Similarly, we can pull subsets in \(Y\) back to subsets
in \(X\) by combining the preimages of every output point
...
\captionsetup{labelsep=none}\includegraphics{figures/set_transformations/set_pushforward/set_pushforward.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/set_transformations/set_pullback/set_pullback.pdf}
...
the output space \(\mathsf{y} \subset Y\) can be pulled back into
subsets of the input space, \(f^{*}(\mathsf{y})\subset X\).}

\end{figure}%

There is an asymmetry, however, between these two induced
transformations when we consider set operations. Both the pushforward
and pullback maps are compatible with the union operation,
\begin{align*}
...
pushes forward to a \emph{subset} of the intersection of the individual
...
\captionsetup{labelsep=none}\includegraphics{figures/binary_op_consistency/inputs/inputs.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/binary_op_consistency/union_pushforward/union_pushforward.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/binary_op_consistency/intersection_pushforward/intersection_pushforward.pdf}
...
generally only a subset of the pushforward of the intersection of the
two input subsets. In other words, the pushforward operation commutes
with the union operator, allowing us to apply them in either order and
...
\(\mathcal{Y}\); we can safely ignore some measurable input subsets
without compromising the \(\sigma\)-algebra on the output space.
...
5} when I introduced measure-informed integration. A real-valued
function \(f : X \rightarrow \mathbb{R}\) can be integrated on the
measure space \((X, \mathcal{X}, \mu)\) if every half-open interval on
the output space pulls back to a measurable subset on the input space,
\[
...
relevant spaces. Consequently, a function \(f : X \rightarrow Y\)
mapping the Borel measurable space \((X, \mathcal{B}_{X})\) into the
Borel measurable space \((Y, \mathcal{B}_{Y})\) might be described as
...
a much weaker condition than topological continuity; we can map open
subsets in the output space into not only open subsets in the output
space, but also closed subsets in the output space, and even any subset
that we can derive from unions and intersections of open and closed
...
measurable. Infinite-dimensional spaces are another matter, but those
spaces will largely be outside of the scope of this book.

\section{Transforming Measures}\label{transforming-measures}

Conveniently, the pullback of measurable subsets allows us to
pushforward measures from the input space to a compatible measure on the
output space. Given a \((\mathcal{X}, \mathcal{Y})\)-measurable function
...
subsets pull back to measurable input subsets along measurable
functions, we can push forward measure allocations. In particular, the
pushforward measure allocated to the measurable output subset
...
At the same time, certain transformations can endow their corresponding
...
corresponding rows
(Figure~\ref{fig-finite-marginal-initial-allocations},
...
\captionsetup{labelsep=none}\includegraphics{figures/finite_marginal/function/function.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/finite_marginal/table/table.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/finite_marginal/initial_allocations/initial_allocations.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/finite_marginal/marginal_allocations/marginal_allocations.pdf}
...
Historically, this kinds of graphical organization motivated the term
...
\captionsetup{labelsep=none}\includegraphics{figures/continuous_marginal/level_sets/level_sets.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/continuous_marginal/pullback_sets/pullback_sets.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/continuous_marginal/marginal_probability/marginal_probability.pdf}
...
subset (Figure~\ref{fig-projection}). Pushing measures forward along
...
\captionsetup{labelsep=none}\includegraphics{figures/projection/level_sets/level_sets.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/projection/pullback_sets/pullback_sets.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/projection/marginal_probability/marginal_probability.pdf}
...
maps an input space into a subset of itself, \(Y \subset X\),
...
to the points in that subset. (c) The pushforward measure collapses the
measure allocated to each level set down to the corresponding anchor
point.}

\end{figure}%

Consider for instance, the unit interval interpreted as a subset of a
real line, \[
(0, 1) \subset R.
...
When pushing measures forward along the logistic function, the measure
...
entire real line into a unit interval. When we push an input measure
forward along the logistic function, the measure allocated to an input
interval is squeezed into a narrower interval.}
...
\hyperref[sec:terminology]{Section 2.1}. A measure \(\mu\) on the input
...
\end{align*} Because the input elements \(\clubsuit\) and \(\triangle\)
both map to the same output element, their atomic allocations are
combined into a single pushforward atomic allocation.
...
interval \((n - 1, n]\) to the integer \(n\), and the atomic subsets on
the output space pull back to those input intervals
...
next-largest integer defines a map from a real line to the integers. The
input measure allocated to the half-open interval \((n - 1, n]\)
projects down to the corresponding integer \(n\).}

\end{figure}%

The Lebesgue measure on the input real line pushes forward along \(f\)
to a measure on the integers with the atomic allocations \begin{align*}
f_{*} \lambda( \{ n \} )
&=
\lambda( f^{*} ( \{ n \} ) )
\\
...
That said, the counting measure on the output space does not provide any
information about how to distribute the atomic allocation
\(\chi( \{ n \} ) = 1\) across the entire pullback subset
\((n - 1, n]\). Instead, there are infinitely many ways that the measure
...
As we saw in \hyperref[sec:pushforward-sets]{Section 1}, pushforward
sets do not generally respect the intersection set operator, preventing
us from pushing \(\sigma\)-algebras forward from the input space to the
output space. Pushing sets forward along injective functions, however,
does respect the intersection operator. Consequently, we can always push
\(\sigma\)-algebras forward along injective functions.

When working with measurable spaces that are already equipped with
\(\sigma\)-algebras, \((X, \mathcal{X})\) and \((Y, \mathcal{Y})\), any
injective function \(f : X \rightarrow Y\) that both pulls measurable
output subsets back to measurable input subsets, \[
f^{*}(\mathsf{y}) \subset \mathcal{X},
\] \emph{and} pushes measurable input subsets forward to measurable
output subsets, \[
...
Because the pushforward and pullback maps along any injective function
always satisfy \[
f^{*} \circ f_{*} = I,
\] we also have \[
...
any measure on the input space,
\(\mu : \mathcal{X} \rightarrow [0, \infty]\), forward to a
corresponding measure on the output space,
\(\mu_{*} : \mathcal{Y} \rightarrow [0, \infty]\), with the allocations
\[
\mu_{*}(\mathsf{y}) = \mu( \, f^{*}(\mathsf{y}) \, ).
\] Bimeasurable functions, however, also allow us to pull any measure on
the output space, \(\nu : \mathcal{Y} \rightarrow [0, \infty]\), back to
a corresponding measure on the input space,
\(\nu^{*} : \mathcal{X} \rightarrow [0, \infty]\), with the allocations
\[
\nu^{*}(\mathsf{x}) = \nu( \, f_{*}(\mathsf{x}) \, ).
\]

When a function \(f : (X, \mathcal{X}) \rightarrow (Y, \mathcal{Y})\) is
not only bimeasurable but also bijective, these operations are
consistent with each other. Pulling a pushforward measure back always
...
&=
\mu( \mathsf{x} ),
\end{align*} and pushing a pullback measure forward always recovers the
...
measure from its pushforward allocations, and any output measure from
its pullback allocations. Bimeasurable functions do not lose
information!
...
bimeasurable bijection that transforms any discrete measure into
another, equivalent discrete measure. In practice, this means that we
are free to choose the permutation that yields the most convenient
organization of the underlying set, without having to worry about
compromising the information encoded in the original space!

Similarly, every continuous bijection between two real lines is
bimeasurable with respect to the Borel \(\sigma\)-algebras.
Consequently, continuous bijections transform any measure over a rigid
real line into a measure on another rigid real line without losing
information. Equivalently, these maps transform a measure over a
parameterization of a flexible real line into a measure over another
...
system that are most convenient in a given application, without having
to worry about affecting the measure structure.
...
output space and then relate them to integrals with respect to the
...
words, integrands on the output space always pull back along measurable
transformations to integrands on the input space.
...
input space, \[
\phi^{*} f = f \circ \phi,
\] and then integrating with respect to \(\mu\). So long as we know how
to compute \(\mu\)-informed integrals, we can compute
...
function \(f : (X, \mathcal{X}) \rightarrow
     (\mathbb{R}, \mathcal{B}_{\mathbb{R}})\). If \(f\) is
\(\mu\)-integrable, then we can integrate \(f\) with respect to \(\mu\)
to give the single real number \(\mathbb{I}_{\mu}[f] \in \mathbb{R}\).
On the other hand, we can also push \(\mu\) forward along \(f\) to
construct a pushforward measure \(f_{*} \mu\) over the possible output
values!
...
Personally, I try to avoid these terminologies because I find that they
...
thoroughly in \hyperref[sec:1d-pushforward-characterizations]{Section
5}.
...
subset allocations directly. In most applications we define probability
distributions implicitly using probability density functions with
respect to a convenient reference measure. To transform probability
distributions in this applied context, we will need to know how to push
probability density functions forward along measurable transformations.
...
\] Given a function
\(f : (X, \mathcal{X}) \rightarrow (Y, \mathcal{Y})\), we can also push
both \(\pi\) and \(\nu\) forward to measures on the output space, and
then construct the probability density function between them, \[
...
\(\lambda\), then the relevant output probability density function is no
longer \[
\frac{ \mathrm{d} f_{*} \pi }{ \mathrm{d} f_{*} \nu} :
Y \rightarrow \mathbb{R}^{+}.
\] but rather \[
...
respect to \(\lambda\), then we can expand this output probability
...
\cdot
\frac{ \mathrm{d} f_{*} \pi }{ \mathrm{d} \lambda}(y).
\] When \(f\) is measurable \emph{and} bijective, we can derive the
...
That said, the computation is made a bit more straightforward by
considering the transformation properties of measure-informed integrals,
\begin{align*}
\mathbb{I}_{\nu}[g \circ f]
&=
\mathbb{I}_{f_{*} \nu}[g]
...
space to \(\lambda\)-informed integration out the output space, then
this relationship may allow us to compute the warping factor
\(\mathrm{d} f_{*} \nu / \mathrm{d} \lambda\).

Constructing pushforward probability density functions along
non-bijective functions is more complicated. Ultimately, we need to
...
Despite all of these equations, however, this is largely an informal
argument. Formalizing it, in particular defining exactly what the
measures across the level sets \(\kappa_{y}(\mathrm{d}z)\) need to be to
ensure consistent results, is a subtle mathematical problem.
Conveniently, we'll tackle this exact problem when we introduce
\emph{conditional} probability theory in the
\href{http://www.symplectomorphic.com/assets/chapters_html/conditional_probability_theory_1.html}{next
chapter}.
...
compute their transformation properties directly by applying the
...
integration with respect to a counting measure reduces to discrete
...
\end{align*} When \(f : X \rightarrow Y\) is bijective, there will be
one, and only one, input point \(x \in X\) with \(f(x) = y'\) and \[
\mathbb{I}_{ \chi_{X} } [ I_{ \{ y' \} } \circ f ]
=
\sum_{x \in X} I_{ \{ y' \} }(f(x))
=
...
Combining these two results gives \begin{align*}
\mathbb{I}_{ \chi_{X} } [ I_{ \{ y' \} } \circ f ]
&=
\mathbb{I}_{ f_{*} \chi_{X} } [ I_{ \{ y' \} } ]
\\
...
\(f^{-1}( y )\) first, and then sum over those contributions, \[
...
\\
&= p \circ f^{-1}(y).
\end{align*} This is consistent with what we derived above.
...
by all of the product elements with \(n\) component elements equaling
one and \(N - y\) component elements equaling zero. Because there are \[
...
The pushforward probability mass function is given by applying the
general transformation rule, \begin{align*}
s_{*} p(y)
&=
\sum_{x \in s^{-1}(y) } p(x)
\\
...
Unfortunately, integration on real spaces cannot be performed with
summation. Consequently, computing the warping factor is much more
complicated.
...
\(f : (X, \mathcal{B}_{\mathbb{R}^{N}}) \rightarrow
     (Y, \mathcal{B}_{\mathbb{R}^{N}})\). Moreover, let \(\lambda_{X}\)
and \(\lambda_{Y}\) be the Lebesgue measures on \(X\) and \(Y\),
respectively.

In this case, the transformation rule for the measure-informed integral
of an integrand \(g : Y \rightarrow \mathbb{R}\) is given by
\begin{align*}
...
\right].
\end{align*} If \(g\) is sufficiently well-behaved, then this reduces to
a relationship between Riemann integrals, \[
...
determinant, it is also known as a \textbf{Jacobian correction}.
...
\(1 / \mathrm{det} \, \mathbf{J}_{f}(f^{-1}(y))\) is a mouthful. It also
quantifies how warped the input Lebesgue measure appears relative to the
output Lebesgue measure. If the warping factor is smaller than one, then
the input volumes appear to expand. Similarly, if the warping factor is
larger than one, then the input volumes appear to contract.}

\end{figure}%

A nearly universal convention in applied probability theory is to ignore
the fact that this relationship has to hold for only
\(\lambda_{Y}\)-almost all \(y \in Y\) and just take the warping factor
to be \[
\frac{ \mathrm{d} f_{*} \lambda_{X} }{ \mathrm{d} \lambda_{Y}}(y)
=
\frac{1}{ | \mathrm{det} \, \mathbf{J}_{f}(f^{-1}(y)) |}.
...
the reciprocal of the Jacobian determinant, mistakenly using \[
f_{*} p(y)
\overset{\lambda_{Y}}{=}
p(f^{-1}(y)) \, | \mathrm{det} \, \mathbf{J}_{f}(f^{-1}(y)) |.
\] To avoid this latter mistake, I rely on a mneumonic to help me
remember the proper orientation of the Jacobian determinant. I start
with the informal integral relationship, \[
\int \mathrm{d} y \, f_{*} p(y) = \int \mathrm{d} x \, p(x),
\] drop the integral signs, \[
\mathrm{d} y \, f_{*} p(y) = \mathrm{d} x \, p(x),
\] and then \emph{very} informally divide by the differentials,
\begin{align*}
\mathrm{d} y \, f_{*} p(y) &= \mathrm{d} x \, p(x)
\\
f_{*} p(y)
...
extremely mathematically sloppy -- but the result can be quickly derived
and always gives the correct orientation of the Jacobian correction.

Is there anything we can do about non-bijective functions? We might, for
instance, be tempted to utilize the Dirac delta function to try to
define iterated expectation values on the input space, \begin{align*}
...
\href{http://www.symplectomorphic.com/assets/chapters_html/conditional_probability_theory_1.html}{next
chapter}, we'll see how we can formalize this strategy using
\emph{conditional} expectation values to implement integration over
level sets.

\subsubsection{Examples}\label{examples}

After all of that discussion, let's put these results into practice with
...
A translation operator is a function that maps each point in the input
real line to a translated point in the output real line,
\begin{alignat*}{6}
...
density function along a translation operator becomes \begin{align*}
...
A scaling operator is a function that maps each point in the input space
to a scaled point in the output space, \begin{alignat*}{6}
...
The pushforward of a normal density function along a scaling operator is
...
along a scaling operator results in another normal probability density
...
composite function gives \begin{align*}
...
\] In other words, we can generate \emph{every} normal probability
density function from a suitable transformation of a unit normal
...
that we introduced in \hyperref[sec:terminology]{Section 2.1},
\begin{alignat*}{6}
...
\end{align*} The Jacobian correction for this transformation is then
\begin{align*}
\frac{1}
...
is given by (Figure~\ref{fig-logistic}) \begin{align*}
...
Probability distributions on high-dimensional spaces are overwhelming
objects that are typically too difficult to study directly. For example,
we cannot convey the entirety of a high-dimensional probability
distribution in a single visualization.

We can, however, explore high-dimensional behavior by examining multiple
low-dimensional summaries, not unlike a carpenter checking if a piece of
wood is warped by staring down each edge one at a time
(Figure~\ref{fig-projective-viewing}). More formally, we can push a
high-dimensional probability distribution forward to many
one-dimensional probability distributions that can be neatly visualized.
...
\captionsetup{labelsep=none}\includegraphics{figures/projective_viewing/uninformative_perspective/uninformative_perspective.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/projective_viewing/informative_perspectives/informative_perspectives.pdf}
...
Given a probability space \((X, \mathcal{X}, \pi)\) and a collection of
...
summary function \(f_{n}\) are, the more interpretable the corresponding
pushforward distribution will be.
...
summary functions. Unfortunately, in the
\href{http://www.symplectomorphic.com/assets/chapters_html/conditional_probability_theory_1.html}{next
chapter} we'll see that the necessary operations are often infeasible in
practice.
...
probability density function is particularly useful for visualizing
pushforward behavior extracted from a high-dimensional input probability
distribution. Frustratingly, they often infeasible to construct in
practice.}
...
That said, in
\href{http://www.symplectomorphic.com/assets/chapters_html/expectation_values.html}{Chapter
Five} we learned multiple ways to characterize one-dimensional
probability distributions using only expectation values. If we can
compute expectation values on the initial, high-dimensional ambient
space, then we can use the pullback of expectation values, \[
\mathbb{E}_{f_{*} \pi}[ g ] = \mathbb{E}_{\pi} [ g \circ f ].
\] to implement these expectation-based characterizations in practice.

For instance, in \hyperref[sec:transforming-integrals]{Section 3} we saw
that the moments of one-dimensional pushforward distributions can be
evaluated as expectation values over the initial space. In particular,
we can compute the mean and variance of the pushfoward probability
distribution \((f_{n})_{*} \pi\) by evaluating the expectation values \[
...
\end{align*} These pushforward interval probabilities allow us to
construct histogram representations of each pushforward probability
distribution (Figure~\ref{fig-pushforward-characterizations-histogram}).
The narrower the pushforward intervals we use, the more these histograms
convey the same information that a pushforward probability density
function would, especially when we scale the bin probabilities by the
interval lengths as we discussed in
\href{http://www.symplectomorphic.com/assets/chapters_html/density_functions.html\#lebesgue-probability-densities-as-limiting-interval-probabilities}{Chapter
Six, Section 4.4}.
...
and typical computational budgets afford for only a finite number. In
practice, we have to be content with \emph{approximating} pushforward
...
\caption{\label{fig-pushforward-characterizations-cdf}Aggregating
pushforward bin probabilities allows us to approximately visualize a
pushforward cumulative distribution function.}

\end{figure}%

Lastly, we can often characterize one-dimensional pushforward
probability distribution with nested pushforward quantile intervals
...
\captionsetup{labelsep=none}\includegraphics{figures/pushforward_characterizations/quantile_ribbons/discrete/discrete.pdf}
...
\captionsetup{labelsep=none}\includegraphics{figures/pushforward_characterizations/quantile_ribbons/continuous/continuous.pdf}
...
the summary function outputs. With that caveat in mind, however, they
...
ability to actually implement these transformations in practice, is a
critical tool in applied probability theory. These transformations allow
us to not only manipulate probability spaces into more convenient forms,
but also distill high-dimensional probabilistic information into
low-dimensional summaries that are straightforward to visualize.
