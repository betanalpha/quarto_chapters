---
title: "Probability Theory on General Spaces"
...
**measurable sets** while any subsets in the power set but not in the
...
of the open and closed subsets.
...
$\sigma$-algebra to remove any undesired subsets that would obstruct the
...
be more convenient.  In more applied practice we can safely assume a
Borel $\sigma$-algebra or any extension that might be needed to avoid
...
infinitely large databases linking each measurable subset to their
allocations is not particularly practical!
...
need to specify only the allocations to atomic subsets, and in some
...
introduce two of the most important algorithm representations of
